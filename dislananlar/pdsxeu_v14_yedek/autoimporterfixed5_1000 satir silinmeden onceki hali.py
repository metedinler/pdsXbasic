...
def get_networkx():
    """NetworkX'i lazy loading ile yükle"""
    global nx
    if nx is None:
        try:
            import networkx as networkx_lib
            nx = networkx_lib
            print("[PDS-X] BÝLGÝ: 'networkx' baþarýyla yüklendi.")
        except ImportError:
            print("[PDS-X] UYARI: 'networkx' yüklenemedi. Geliþmiþ baðýmlýlýk optimizasyonu devre dýþý.")
            nx = False # Tekrar denenmemesi için False olarak iþaretle
    return nx
...
# JSONL loglarý için özel formatlayýcý
class JsonLogFormatter(logging.Formatter):
    """JSONL dosyasý için log kayýtlarýný JSON formatýna dönüþtürür."""
    def format(self, record):
        ...
        # record nesnesini doðrudan deðiþtirmek yerine formatlanmýþ string'i oluþtur
        log_record = {
            "time": self.formatTime(record, self.datefmt),
            "level": record.levelname,
            "message": safe_message
        }
        # ensure_ascii=False Türkçe karakterlerin doðru yazýlmasýný saðlar
...
class AdvancedLogger:
    ...
    def __init__(self, log_dir: Path = LOG_DIR, terminal_log_path: Path = TERMINAL_LOG_FILE, jsonl_log_path: Path = JSONL_LOG_FILE):
        ...
        # Özel JSON formatlayýcýyý kullan
        self.jsonl_handler.setFormatter(JsonLogFormatter())
    ...
    def log(self, level: str, message: str, *args, **kwargs):
        ...
        # Mesaj temizleme iþlemi kaldýrýldý, orijinal mesaj doðrudan loglanýr.
    ...
    def shutdown(self):
        ...
        for task in reversed(self.tasks):
            try:
                ...
                # Hata mesajýný logla ve ardýndan tam traceback'i de logla.
                error_message = f"'{task[\'name\']}' görevi durdurulurken hata oluþtu: {e}"
                # exc_info=True parametresi, log kaydýna exception traceback'ini ekler.
                self.logger.log("error", error_message, exc_info=True)
...
class DependencyOptimizer:
    ...
    def __init__(self, logger: AdvancedLogger, dependency_registry: 'DependencyRegistry', heuristic_manager: 'HeuristicManager'):
        ...
        # self.graph artýk burada oluþturulmuyor, her optimizasyonda taze olarak alýnacak.
    ...
    def get_optimized_order(self, packages_to_install: List[str]) -> List[str]:
        ...
        # 1. Adým: Grafiði her seferinde taze olarak oluþtur.
        graph = self._build_dependency_graph()

        # 2. Adým: Kurulacak tüm paketleri ve bunlarýn bilinen tüm baðýmlýlýklarýný topla.
        ...
        # 3. Adým: Toplanan tüm paketler üzerinde topolojik sýralama yap.
        sorted_order = []
        visited = set()
        
        for pkg_name in list(all_packages_to_consider):
            if pkg_name not in visited:
                if not self._topological_sort_util(pkg_name, visited, set(), sorted_order, graph):
                    self.logger.log("warning", "Baðýmlýlýk grafiðinde döngü tespit edildi! Optimizasyon atlanýyor.")
                    return packages_to_install

        # 4. Adým: Son listeyi oluþtur, orijinal versiyonlarý geri yükle.
    ...
    def _topological_sort_util(self, package: str, visited: set, recursion_stack: set, sorted_order: List[str], graph: Dict[str, List[str]]):
        """Topolojik sýralama için yardýmcý DFS fonksiyonu."""
        visited.add(package)
        recursion_stack.add(package)

        # Bu paketin baðýmlýlýklarýný (güncel grafikten) gez
        dependencies = graph.get(package, [])
        for dep in dependencies:
            if dep not in visited:
                if not self._topological_sort_util(dep, visited, recursion_stack, sorted_order, graph):
...
class ConflictManager:
    ...
    def __init__(self, logger, dependency_registry, env_manager):
        self.logger = logger
        self.dependency_registry = dependency_registry
        self.env_manager = env_manager
    ...
    def check_conflicts(self, package_requirement: str) -> Optional[str]:
        ...
        try:
            ...
            Version = self.packaging_libs["Version"]
            req = Requirement(package_requirement)
            
            package_info = self.dependency_registry.get_package_info(req.name)
            if package_info:
                installed_version = Version(package_info.get("current_version", "0.0.0"))
                if not req.specifier.contains(installed_version, prereleases=True):
                    conflict_msg = (
                        f"'{package_requirement}' paketi, halihazýrda kurulu olan "
                        f"'{req.name}=={installed_version}' ile çakýþýyor."
                    )
                    self.logger.log("warning", conflict_msg)
                    return conflict_msg
    ...
    def check_and_log_conflicts(self):
        """Tüm ortamdaki baðýmlýlýklarý 'pip check' ile kontrol eder ve loglar."""
        if not self.env_manager.pip_path:
            self.logger.log("error", "Pip yolu bulunamadýðý için tam çakýþma kontrolü yapýlamýyor.")
            return
            
        cmd = [str(self.env_manager.pip_path), "check"]
        self.logger.log("info", f"Tam baðýmlýlýk kontrolü çalýþtýrýlýyor: {' '.join(cmd)}")
        
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='ignore', check=False)
            
            if result.returncode != 0:
                self.logger.log("warning", "--- BAÐIMLILIK ÇAKIÞMASI TESPÝT EDÝLDÝ ---")
                self.logger.log("warning", result.stdout)
                self.logger.log("warning", "--- ÇAKIÞMA RAPORU SONU ---")
            else:
                self.logger.log("info", "? Baðýmlýlýk kontrolü tamamlandý. Herhangi bir çakýþma bulunamadý.")
        except FileNotFoundError:
            self.logger.log("error", f"'{self.env_manager.pip_path}' komutu bulunamadý. Pip doðru þekilde ayarlanmamýþ olabilir.")
        except Exception as e:
            self.logger.log("error", f"Baðýmlýlýk kontrolü sýrasýnda beklenmedik bir hata oluþtu: {e}")
...
class SmartInstallManager:
    """
    Chaos analizi, adaptif önceliklendirme, asenkron kurulum, baðýmlýlýk optimizasyonu,
    blockchain tabanlý doðrulama ve çakýþma geri alma adýmlarýný içerir.
    """
    def __init__(self, modules: List[str], installer: 'AutoImporter'):
        self.modules = modules
        self.installer = installer
        self.chaos_report: Dict[str, Any] = {}
        self.priority_queue: List[str] = []
        self.dependency_order: List[str] = []
        self.final_queue: List[str] = []

    # Adým 1: Chaos Analizi
    def analyze_chaos(self, duration: int = 10):
        cpu, mem = [], []
        for _ in range(duration):
            cpu.append(psutil.cpu_percent(interval=1))
            mem.append(psutil.virtual_memory().percent)
        cpu_std = round(stdev(cpu), 2) if len(set(cpu)) > 1 else 0.0
        mem_std = round(stdev(mem), 2) if len(set(mem)) > 1 else 0.0
        chaos_index = round((cpu_std + mem_std) / 2, 2)
        status = (
            "Düþük Yük (Kararlý)" if chaos_index < 3 else
            "Dalgalý Yük (Orta)" if chaos_index < 7 else
            "Kaotik Yük (Yüksek)"
        )
        try:
            _, p_value = f_oneway(cpu, mem)
            similar = (p_value or 0) > 0.05
        except Exception:
            p_value, similar = None, False
        self.chaos_report = {
            "cpu_std": cpu_std, "mem_std": mem_std,
            "chaos_index": chaos_index,
            "status": status, "p_value": p_value,
            "similar_variance": similar
        }
        self.installer.logger.log("info", f"?? Chaos Raporu: {json.dumps(self.chaos_report)}")

    # Adým 2: Adaptive Önceliklendirme
    def prioritize_adaptive(self):
        level = self.chaos_report.get("status", "Düþük Yük (Kararlý)")
        def weight(mod: str) -> int:
            w = 0
            name = mod.lower()
            if any(x in name for x in ["torch", "tensorflow"]):
                w += 3
            elif any(x in name for x in ["cv2", "image"]):
                w += 2
            elif "numpy" in name:
                w += 1
            # Chaos level bonus
            if level == "Kaotik Yük (Yüksek)":
                w += 2
            elif level == "Dalgalý Yük (Orta)":
                w += 1
            return w
        self.priority_queue = sorted(self.modules, key=weight)
        self.installer.logger.log("info", f"?? Adaptive Sýralama: {self.priority_queue}")

    # Adým 3: Dependenciy Optimizasyonu
    def optimize_dependencies(self):
        graph = nx.DiGraph()
        # Basit örnek baðýmlýlýk haritasý
        dep_map = {
            "scipy": ["numpy"], "pandas": ["numpy"], "matplotlib": ["numpy"],
            "sklearn": ["scipy", "numpy"], "tensorflow": ["numpy"],
            "torch": ["numpy"], "cv2": ["numpy"]
        }
        for mod in self.priority_queue:
            graph.add_node(mod)
            for dep in dep_map.get(mod.lower(), []):
                if dep in self.priority_queue:
                    graph.add_edge(dep, mod)
        try:
            self.dependency_order = list(nx.topological_sort(graph))
        except nx.NetworkXUnfeasible:
            self.installer.logger.log("warning", "?? Döngüsel baðýmlýlýk tespit edildi.")
            self.dependency_order = self.priority_queue
        self.installer.logger.log("info", f"?? Dependency Sýrasý: {self.dependency_order}")

    # Adým 4: Asenkron Kurulum
    async def schedule_installation(self):
        tasks = []
        self.final_queue = self.dependency_order or self.priority_queue
        for mod in self.final_queue:
            delay = self.get_delay(mod)
            tasks.append(self.install_with_delay(mod, delay))
        
        # gather sonuçlarý, her bir install_with_delay çaðrýsýndan dönen yeni baðýmlýlýk listelerini içerir.
        results = await asyncio.gather(*tasks)
        
        # Toplanan tüm yeni baðýmlýlýklarý tek bir listede birleþtir
        new_dependencies = [dep for sublist in results for dep in sublist if sublist]
        if new_dependencies:
            self.installer.logger.log("info", f"Asenkron kurulum sýrasýnda yeni baðýmlýlýklar tespit edildi: {new_dependencies}")
            # Bu yeni baðýmlýlýklarý da kurmak için ana kurulum döngüsüne eklemek gerekebilir.
            # Þimdilik sadece logluyoruz, daha karmaþýk bir yapý için burasý geniþletilebilir.
            # Örneðin: self.installer.install_from_list(new_dependencies)

    async def install_with_delay(self, mod: str, delay: float) -> List[str]:
        await asyncio.sleep(delay)
        try:
            # Eski install_package yerine yeni akýllý deneme mekanizmasýný kullan
            new_deps = self.installer._install_package_with_retry(mod)
            return new_deps
        except Exception as e:
            self.installer.logger.log("error", f"Kurulum hatasý ({mod}): {e}")
            return [] # Hata durumunda boþ liste döndür

    def get_delay(self, mod: str) -> float:
        base = random.random()
        level = self.chaos_report.get("status", "Düþük Yük (Kararlý)")
        if level == "Kaotik Yük (Yüksek)":
            base += 3
            ...
            base += 1
        if any(x in mod.lower() for x in ["torch", "tensorflow"]):
            base += 2
        return base

    # Adým 5: Çakýþma Tespiti ve Geri Dönüþ
    def detect_conflicts_and_rollback(self):
        """pip check ile baðýmlýlýklarý kontrol eder ve çakýþma varsa geri almaya çalýþýr."""
        if not self.installer.env_manager.pip_path:
            self.installer.logger.log("error", "Pip yolu bulunamadýðý için çakýþma kontrolü yapýlamýyor.")
            return
            
        cmd = [str(self.installer.env_manager.pip_path), "check"]
        self.installer.logger.log("info", f"Baðýmlýlýk kontrolü çalýþtýrýlýyor: {' '.join(cmd)}")
        
        result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='ignore')
        
        if result.returncode != 0:
            self.installer.logger.log("warning", f"pip check komutu potansiyel çakýþmalar buldu:\n{result.stdout}")
            for line in result.stdout.splitlines():
                if not line.strip():
                    continue
                self.installer.logger.log("warning", f"?? Çakýþma: {line}")
                # pip check çýktýsý 'package-name version has requirement other-package ...' þeklinde olabilir.
                pkg = line.split()[0]
                prev = self.get_previous_version(pkg)
                if prev:
                    self.installer.logger.log("info", f"?? Geri alýnýyor: {pkg}=={prev} kuruluyor...")
                    try:
                        rollback_cmd = [str(self.installer.env_manager.pip_path), "install", "--force-reinstall", f"{pkg}=={prev}"]
                        rollback_result = subprocess.run(
                            rollback_cmd,
                            capture_output=True, text=True, check=True, encoding='utf-8', errors='ignore'
                        )
                        self.installer.logger.log("info", f"Geri alma baþarýlý ({pkg}=={prev}): {rollback_result.stdout}")
                    except subprocess.CalledProcessError as e:
                        self.installer.logger.log("error", f"Geri alma baþarýsýz ({pkg}=={prev}): {e.stderr}")
        else:
            self.installer.logger.log("info", "? pip check: Baðýmlýlýk çakýþmasý bulunamadý.")


    def get_previous_version(self, pkg: str) -> Optional[str]:
        """Bir paketin kayýtlý bir önceki versiyonunu döndürür."""
        try:
            package_info = self.installer.dependency_registry.get_package_info(pkg)
            if not package_info:
                self.installer.logger.log("warning", f"'{pkg}' için kayýt bulunamadý, önceki versiyon alýnamýyor.")
                return None
            
            versions = package_info.get("versions", [])
            # Mevcut versiyon listedeki son versiyondur. Bir önceki, sondan ikincidir.
            if len(versions) > 1:
                previous_version = versions[-2]
                self.installer.logger.log("info", f"'{pkg}' için önceki versiyon bulundu: {previous_version}")
                return previous_version
            else:
                self.installer.logger.log("info", f"'{pkg}' için sadece bir versiyon kaydý var, geri alýnacak versiyon yok.")
                return None
        except (AttributeError, IndexError, TypeError) as e:
            self.installer.logger.log("error", f"'{pkg}' için önceki versiyon alýnýrken kritik hata: {e}")
            return None

    # Tam Akýþ
    def run_all(self):
        self.analyze_chaos()
        self.prioritize_adaptive()
        self.optimize_dependencies()
        asyncio.run(self.schedule_installation())
        self.detect_conflicts_and_rollback()
...
class AutoImporter:
    ...
    def __init__(self, mode: str = 'NORMAL', monitor_terminal: bool = False, replay_args=None):
        ...
        self.conflict_manager = ConflictManager(self.logger, self.dependency_registry, self.env_manager)
    ...
    def _install_package_with_retry(self, package: str, max_retries: int = 3) -> List[str]:
        ...
        while retries < max_retries:
            ...
            if install_result["returncode"] == 0:
                ...
                main_pkg = success_details.get("main_package")
                version = success_details.get("version")
                deps = success_details.get("dependencies", [])

                if main_pkg and version:
                    self.dependency_registry.add_package(main_pkg, version, deps)
                    # Baþarýlý kurulumdan öðren
                    self.heuristic_manager.learn_from_success(main_pkg, deps)
                else:
                    # Analiz baþarýsýz olsa bile ana paketi kaydet
                    self.dependency_registry.add_package(pkg_name_only, "latest", [])
                    self.heuristic_manager.learn_from_success(pkg_name_only, []) # Baðýmlýlýk olmasa da öðren
    ...
    def install_from_list(self, packages: List[str]):
        ...
        smart_manager = SmartInstallManager(modules=packages, installer=self)
        try:
            # run_all() tüm adýmlarý (analiz, optimizasyon, kurulum, kontrol) yönetir.
            smart_manager.run_all()
            self.logger.log("info", "Akýllý Kurulum Yöneticisi baþarýyla tamamlandý.")
        except Exception as e:
