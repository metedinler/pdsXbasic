...
try:
    ...
    # Fallback: dummy async file opener if aiofiles not installed
...
# Ortam kontrolü: Doğru sanal ortamda mıyız?
def _check_pdsx_env():
    env_name = os.environ.get("CONDA_DEFAULT_ENV") or os.environ.get("VIRTUAL_ENV")
    python_path = sys.executable
    expected_env = ".pdsx_isolated_env"
    if env_name and expected_env not in env_name and expected_env not in python_path:
        msg = f"[PDS-X] UYARI: Şu anda '{env_name or python_path}' ortamındasınız. Doğru ortam: '{expected_env}'. Bağımlılık ve modül hataları oluşabilir."
        print(msg)
        log.warning(msg)
    elif not env_name and expected_env not in python_path:
        msg = f"[PDS-X] UYARI: Sanal ortam algılanamadı. Aktif Python: {python_path}. Doğru ortam: '{expected_env}'."
        print(msg)
        log.warning(msg)
...
class PdsXPipeError(PdsXException):
    """PDS-X Boru hatası"""
    def __init__(self, message: str, context: Optional[Dict] = None, error_code: str = "ERR_PIPE_UNKNOWN"):
...
__pdsX_exports__ = {
    "classes": {
        ...
        "PdsXSyntaxError": PdsXSyntaxError,
        "PdsXRuntimeError": PdsXRuntimeError,
        "PdsXTypeError": PdsXTypeError,
        "PdsXValueError": PdsXValueError,
        "PdsXMemoryError": PdsXMemoryError,
        "PdsXIOException": PdsXIOException,
        "PdsXSecurityError": PdsXSecurityError,
        "PdsXNetworkError": PdsXNetworkError,
        "PdsXDatabaseError": PdsXDatabaseError,
        "PdsXLogicError": PdsXLogicError,
        "PdsXConcurrencyError": PdsXConcurrencyError,
