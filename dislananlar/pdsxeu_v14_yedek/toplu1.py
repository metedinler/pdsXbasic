...
class EnvManager:
    ...
    def setup_environment(self) -> bool:
        ...
        try:
            ...
            print(f"\n🎯 {success_count}/{total_count} paket kurulumu tamamlandı!")
            if hasattr(self, 'summary_generator'):
                ...
                print("⚠️ Özet sistemi henüz hazır değil")
...
class ScientificUtils:
    ...
    def analyze_terminal_conflicts(self, terminal_output: str) -> Dict:
        """Terminal çıktısından çakışmaları analiz et"""
        try:
            conflicts = {
                "version_conflicts": [],
            ...
            # Version conflict pattern'leri
            version_patterns = [
                r"ERROR: ([^\s]+) ([^\s]+) has requirement ([^\s]+)([><=!]+)([^\s,]+)",
                r"Could not find a version that satisfies the requirement ([^\s]+)([><=!]+)([^\s,]+)",
                r"No matching distribution found for ([^\s]+)([><=!]+)([^\s,]+)"
            ]
            
            # Dependency conflict pattern'leri
            dependency_patterns = [
                r"([^\s]+) requires ([^\s]+) ([><=!]+)([^\s,]+) but you have ([^\s]+) ([^\s]+)",
                r"incompatible with ([^\s]+) ([^\s]+)"
            ]
            
            # Installation failure pattern'leri
            failure_patterns = [
                r"ERROR: Failed building wheel for ([^\s]+)",
                r"ERROR: Could not install packages due to an EnvironmentError",
                r"Microsoft Visual C\+\+ 14\.0 is required"
            ]
            
            # Terminal output'u analiz et
            ...
            for line in lines:
                ...
                for pattern in version_patterns:
                    ...
                    if match:
                        ...
                        conflicts["version_conflicts"].append(conflict_info)
                        self.logger.log("warning", f"Terminal version conflict: {line}")
                ...
                for pattern in dependency_patterns:
                    ...
                    if match:
                        ...
                        conflicts["dependency_conflicts"].append(conflict_info)
                        self.logger.log("warning", f"Terminal dependency conflict: {line}")
                ...
                for pattern in failure_patterns:
                    ...
                    if match:
                        ...
                        conflicts["installation_failures"].append(failure_info)
                        self.logger.log("error", f"Terminal installation failure: {line}")
            
            # Çakışma sayılarını logla
            total_conflicts = (len(conflicts["version_conflicts"]) + 
                             len(conflicts["dependency_conflicts"]) + 
                             len(conflicts["installation_failures"]))
            
            self.logger.log("info", f"Terminal analizi: {total_conflicts} çakışma tespit edildi")
            
            return conflicts
            
        except Exception as e:
            self.logger.log("error", f"Terminal çakışma analizi hatası: {e}")
            return {"error": str(e)}
    
    def suggest_conflict_resolutions(self, conflicts: Dict) -> List[Dict]:
        """Tespit edilen çakışmalar için çözüm önerileri"""
        try:
            suggestions = []
            
            # Version conflict çözümleri
            for conflict in conflicts.get("version_conflicts", []):
                suggestion = {
                    "conflict_type": "version_conflict",
                    "issue": conflict.get("line", ""),
                    "suggestions": [],
                    "priority": "high"
                }
                ...
                suggestion["suggestions"].extend([
                    ...
                    "Virtual environment oluşturun ve izole kurulum yapın"
                ])
                
                suggestions.append(suggestion)
            
            # Dependency conflict çözümleri
            for conflict in conflicts.get("dependency_conflicts", []):
                suggestion = {
                ...
                suggestion["suggestions"].extend([
                    ...
                    "Dependency resolution için pip-tools kullanın",
                    "requirements.txt dosyasını güncelleyin",
                    "Conflict çözmek için conda kullanmayı deneyin"
                ])
                
                suggestions.append(suggestion)
            
            # Installation failure çözümleri
            for failure in conflicts.get("installation_failures", []):
                suggestion = {
                    "conflict_type": "installation_failure",
                    "issue": failure.get("line", ""),
                    "suggestions": [],
                    "priority": "high"
                }
                
                if "Visual C++" in failure.get("line", ""):
                    suggestion["suggestions"].extend([
                        "Microsoft Visual C++ Build Tools 14.0 kurun",
                        "Visual Studio Community Edition kurun", 
                        "Precompiled wheel kullanın: pip install --only-binary=all <package>"
                    ])
                else:
                    suggestion["suggestions"].extend([
                        "Build dependencies kurun: pip install setuptools wheel",
                        "System packages kurun (apt-get, yum, brew)",
                        "Farklı Python versiyonu deneyin",
                        "Conda ile kurulum yapın"
                    ])
                
                suggestions.append(suggestion)
            
            self.logger.log("info", f"Conflict resolution: {len(suggestions)} öneri oluşturuldu")
            return suggestions
            
        except Exception as e:
            self.logger.log("error", f"Çözüm önerisi oluşturma hatası: {e}")
            return []
    
    def auto_resolve_conflicts(self, conflicts: Dict) -> Dict:
        """Bazı çakışmaları otomatik çözmeye çalış"""
        try:
            resolution_results = {
                "attempted": 0,
                "successful": 0,
            ...
            # Basit version conflict'leri otomatik çöz
            for conflict in conflicts.get("version_conflicts", []):
                try:
                    resolution_results["attempted"] += 1
                    
                    # Conflict'i parse et
                    line = conflict.get("line", "")
                    
                    # Basit force-reinstall dene
                    if "has requirement" in line.lower():
                        # Bu durumda manuel müdahale gerekli
                        action = {
                            "type": "manual_required",
                            "conflict": line,
                            "action": "Manuel çözüm gerekli"
                        }
                        resolution_results["actions_taken"].append(action)
                        resolution_results["failed"] += 1
                    
                except Exception as e:
                    resolution_results["failed"] += 1
                    self.logger.log("error", f"Conflict auto-resolution hatası: {e}")
            
            self.logger.log("info", f"Auto-resolution: {resolution_results['successful']}/{resolution_results['attempted']} başarılı")
            return resolution_results
            
        except Exception as e:
            self.logger.log("error", f"Otomatik çözüm hatası: {e}")
            return {"error": str(e)}

# Birleştirilmiş Modül Kurulum Özet Sistemi
class ModuleSummaryGenerator:
    """
    Paket kurulum işlemlerinin kapsamlı özetini oluşturur ve yazdırır.
    Kurulum başarıları, hataları, istatistikleri ve detaylı modül durumlarını gösterir.
    İki farklı özet sınıfının en iyi özelliklerini birleştirir.
    """
    def __init__(self, logger: AdvancedLogger):
        self.logger = logger
        
        # Ana kurulum istatistikleri (ilk sınıftan)
        ...
        # Detaylı modül durumları (ikinci sınıftan)
        ...
        self.extended_stats = {
            ...
            "fastest_install": {"package": "", "duration": float('inf')},
            "slowest_install": {"package": "", "duration": 0.0},
            "dependency_chains": [],
            "import_tests": [],
            "cache_hits": 0,
            "download_count": 0,
    ...
    def add_success(self, package_name: str, version: str = None, duration: float = 0.0):
        """Başarılı kurulum kaydı - genişletilmiş"""
        install_data = {
        ...
        self.installation_stats["successful"].append(install_data)
        
        # Modül özeti de ekle
        self.add_module_status(package_name, "Başarılı", duration)
        
        # Genişletilmiş istatistikleri güncelle
        self._update_extended_stats(package_name, duration, "success")
    
    def add_failure(self, package_name: str, error: str, duration: float = 0.0):
        """Başarısız kurulum kaydı - genişletilmiş"""
        fail_data = {
            "package": package_name,
            "error": error,
            "duration": duration,
        ...
        self.installation_stats["failed"].append(fail_data)
        
        # Modül özeti de ekle
        self.add_module_status(package_name, "Başarısız", duration)
        
        # Genişletilmiş istatistikleri güncelle
        self._update_extended_stats(package_name, duration, "failure")
    
    def add_skipped(self, package_name: str, reason: str, duration: float = 0.0):
        """Atlanan paket kaydı - genişletilmiş"""
        skip_data = {
            "package": package_name,
            "reason": reason,
            "duration": duration,
        ...
        self.installation_stats["skipped"].append(skip_data)
        
        # Modül özeti de ekle
        self.add_module_status(package_name, "Atlandı", duration)
    ...
    def add_import_test(self, package_name: str, success: bool, duration: float = 0.0):
        """Import testi sonucu ekle"""
        self.extended_stats["import_tests"].append({
            "package": package_name,
            "success": success,
            "duration": duration,
            "timestamp": datetime.now().isoformat()
    ...
    def add_cache_hit(self, package_name: str):
        """Cache hit sayısını artır"""
        self.extended_stats["cache_hits"] += 1
        self.logger.log("debug", f"Cache hit: {package_name}")
    
    def add_download(self, package_name: str):
        """Download sayısını artır"""
        self.extended_stats["download_count"] += 1
        self.logger.log("debug", f"Download: {package_name}")
    
    def add_build(self, package_name: str):
        """Build sayısını artır"""
        self.extended_stats["build_count"] += 1
        self.logger.log("debug", f"Build from source: {package_name}")
    
    def _update_extended_stats(self, package_name: str, duration: float, result_type: str):
        """Genişletilmiş istatistikleri güncelle"""
        self.extended_stats["total_duration"] += duration
        
        # En hızlı ve en yavaş kurulumları takip et
        if result_type == "success" and duration > 0:
            if duration < self.extended_stats["fastest_install"]["duration"]:
                self.extended_stats["fastest_install"] = {"package": package_name, "duration": duration}
            if duration > self.extended_stats["slowest_install"]["duration"]:
                self.extended_stats["slowest_install"] = {"package": package_name, "duration": duration}
    ...
    def finalize_stats(self):
        ...
        total_packages = self.installation_stats["total_packages"]
        if total_packages > 0:
            self.extended_stats["average_duration"] = self.extended_stats["total_duration"] / total_packages
    
    def print_summary(self):
        """Kapsamlı kurulum özetini yazdır - her iki sınıfın özelliklerini birleştirir"""
        try:
            self.finalize_stats()
            
            print("\n" + "="*70)
            print("📦 PDS-X KAPSAMLI PAKET KURULUM ÖZETİ")
            print("="*70)
            ...
            if start_time and end_time:
                ...
                print(f"⏱️  Toplam Kurulum Süresi: {duration.total_seconds():.2f} saniye")
                print(f"⚡ Ortalama Kurulum Süresi: {self.extended_stats['average_duration']:.2f} saniye/paket")
            ...
            # Genişletilmiş performans istatistikleri
            print(f"\n⚡ PERFORMANS İSTATİSTİKLERİ:")
            print(f"   💾 Cache Hit: {self.extended_stats['cache_hits']}")
            print(f"   ⬇️  İndirme: {self.extended_stats['download_count']}")
            print(f"   🔨 Build: {self.extended_stats['build_count']}")
            if self.extended_stats["fastest_install"]["package"]:
                print(f"   🚀 En Hızlı: {self.extended_stats['fastest_install']['package']} ({self.extended_stats['fastest_install']['duration']:.2f}s)")
            if self.extended_stats["slowest_install"]["package"]:
                print(f"   🐌 En Yavaş: {self.extended_stats['slowest_install']['package']} ({self.extended_stats['slowest_install']['duration']:.2f}s)")
            
            # Detaylı modül tablosu (ikinci sınıftan)
            if self.module_summaries:
                print(f"\n📋 DETAYLI MODÜL TABLOSU:")
                header = f"{Fore.CYAN}┌─────────────────────────────┬──────────────┬──────────┐"
                print(header)
                print(f"│ Modül Adı                   │ Durum        │ Süre (sn)│")
                print(f"├─────────────────────────────┼──────────────┼──────────┤{Style.RESET_ALL}")
                
                for s in self.module_summaries:
                    color = Fore.GREEN if "Başarılı" in s["status"] or "Atlandı" in s["status"] else Fore.RED
                    module_name = s['module'][:27] + "..." if len(s['module']) > 27 else s['module']
                    row = f"{color}│ {module_name:<27} │ {s['status']:<12} │ {s['duration']:>8.2f}│{Style.RESET_ALL}"
                    print(row)
                
                footer = f"{Fore.CYAN}└─────────────────────────────┴──────────────┴──────────┘{Style.RESET_ALL}"
                print(footer)
            
            # Başarılı kurulumlar (özet)
            if successful > 0:
                print(f"\n✅ BAŞARILI KURULUMLAR ({successful} adet):")
                for pkg in self.installation_stats["successful"][:10]:  # İlk 10'u göster
                    version_info = f" v{pkg['version']}" if pkg['version'] != 'latest' else ""
                    duration_info = f" ({pkg['duration']:.1f}s)" if pkg['duration'] > 0 else ""
                    print(f"   ✓ {pkg['package']}{version_info}{duration_info}")
                if successful > 10:
                    print(f"   ... ve {successful - 10} paket daha")
            
            # Başarısız kurulumlar
            if failed > 0:
                print(f"\n❌ BAŞARISIZ KURULUMLAR ({failed} adet):")
                for pkg in self.installation_stats["failed"]:
                    error_short = pkg['error'][:50] + "..." if len(pkg['error']) > 50 else pkg['error']
                    print(f"   ✗ {pkg['package']}: {error_short}")
            ...
            # Import testleri
            if self.extended_stats["import_tests"]:
                successful_imports = sum(1 for test in self.extended_stats["import_tests"] if test["success"])
                total_tests = len(self.extended_stats["import_tests"])
                print(f"\n🧪 IMPORT TESTLERİ: {successful_imports}/{total_tests} başarılı")
            
            # Başarı oranı ve değerlendirme
            if total > 0:
                success_rate = (successful / total) * 100
                print(f"\n📈 BAŞARI ORANI: {success_rate:.1f}%")
                
                if success_rate >= 95:
                    print("🎉 Mükemmel! Kurulum kusursuz tamamlandı.")
                elif success_rate >= 90:
                    print("🎊 Harika! Kurulum başarıyla tamamlandı.")
                elif success_rate >= 80:
                    print("👍 İyi! Çoğu paket başarıyla kuruldu.")
                elif success_rate >= 60:
                    print("😐 Orta. Bazı paketlerde sorun var.")
                elif success_rate >= 40:
                    print("� Zayıf. Birçok pakette sorun var.")
                else:
                    print("😞 Kritik! Ciddi kurulum sorunları var.")
            
            print("="*70)
            print("📝 Detaylı log için pdsXu_errors.log dosyasını kontrol edin.")
            print("🔧 Sorun yaşayan paketler için manuel kurulum deneyin.")
            print("="*70 + "\n")
            
            # Log'a kapsamlı özet kaydet
            self.logger.log("info", f"Kapsamlı kurulum özeti: {successful} başarılı, {failed} başarısız, {skipped} atlanan, {self.extended_stats['cache_hits']} cache hit")
    ...
    def get_stats_dict(self) -> Dict:
        """İstatistikleri dictionary olarak döndür"""
        self.finalize_stats()
        return {
            "installation_stats": self.installation_stats,
            "extended_stats": self.extended_stats,
            "module_summaries": self.module_summaries
        }
    
    def save_stats_to_file(self, filepath: str = "installation_summary.json"):
        """İstatistikleri JSON dosyasına kaydet"""
        try:
            stats = self.get_stats_dict()
            with open(filepath, "w", encoding="utf-8") as f:
                json.dump(stats, f, indent=2, ensure_ascii=False)
            self.logger.log("info", f"İstatistikler kaydedildi: {filepath}")
        except Exception as e:
            self.logger.log("error", f"İstatistik kaydetme hatası: {e}")

# Derleme Araçları Kontrolü
def check_build_tools(logger: AdvancedLogger):
    """
    Derleme araçlarını kontrol eder (Visual Studio Build Tools veya MinGW).
    """
    try:
...
def add_to_path(tool_path: str, logger: AdvancedLogger):
    """
    Belirtilen yolu sistem PATH’ine ekler.
    """
    try:
        setx_cmd = f'setx PATH "%PATH%;{tool_path}"'
        process = subprocess.run(setx_cmd, shell=True, check=True)
        shutdown_manager.register_process(process)
...
class AutoImporter:
    ...
    def replay_last_command(self) -> bool:
        ...
        try:
            ...
            if 'current_dir' in locals() and current_dir != os.getcwd():
                os.chdir(current_dir)    # Eksik metodlar - Demo'da kullanılan metodlar
    ...
    def install_package(self, package: str, silent: bool = False) -> bool:
        """
        Paketi kurar ve gelişmiş özet istatistiklerini günceller
        ...
        try:
            ...
            if self.check_package_installed(package_name):
                ...
                self.summary_generator.add_skipped(package_name, "Zaten kurulu", duration)
                return True
            ...
            if result.returncode == 0:
                ...
                # Cache/download/build durumunu tespit et
                if "Using cached" in result.stdout:
                    self.summary_generator.add_cache_hit(package_name)
                elif "Downloading" in result.stdout:
                    self.summary_generator.add_download(package_name)
                elif "Building wheel" in result.stdout:
                    self.summary_generator.add_build(package_name)
                
                self.summary_generator.add_success(package_name, version, total_duration)
                
                # Import testi yap
                import_start = time.time()
                try:
                    # Module adını tespit et ve import et
                    module_mapping = {
                        'pillow': 'PIL',
                    ...
                    test_module = module_mapping.get(package_name, package_name)
                    importlib.import_module(test_module)
                    import_duration = time.time() - import_start
                    self.summary_generator.add_import_test(package_name, True, import_duration)
                except ImportError:
                    import_duration = time.time() - import_start
                    self.summary_generator.add_import_test(package_name, False, import_duration)
                
                if not silent:
                    self.logger.log("info", f"✅ {package_name} başarıyla kuruldu ({total_duration:.2f}s)")
                ...
                self.summary_generator.add_failure(package_name, error_msg, total_duration)
...
class TerminalLogAnalyzer:
    ...
    def extract_missing_imports(self, log_content: str) -> List[str]:
        ...
        try:
            for pattern in self.error_patterns['module_not_found']:
                ...
                self.learned_deps["dependency_errors"] = list(set(self.learned_deps.get("dependency_errors", []) + dependency_errors))
                xception as e:
                # Hash'i cache'e eklef"Eksik import analizi hatası: {e}")
                self.analyzed_hashes.add(file_hash)
                issing_modules
                # Kaydet
    ...
    def extract_dependency_errors(self, log_content: str) -> List[str]:
        """Dependency error'larından paket adlarını çıkar"""
        dependency_errors = []
        
        try:
            for pattern in self.error_patterns['dependency_error']:
                matches = re.findall(pattern, log_content, re.IGNORECASE)
                for match in matches:
                    package = match.strip()
                    if package and package not in dependency_errors:
                        dependency_errors.append(package)
                        self.logger.log("warning", f"Dependency error tespit edildi: {package}")
        except Exception as e:
            self.logger.log("error", f"Dependency error analizi hatası: {e}")
...
# Real-time Log İzleme Sistemi
            ...
            self.save_learned_dependencies()
            self.logger.log("info", "Öğrenilmiş bağımlılıklar ve cache temizlendi")
        except Exception as e:dependencies()
            self.logger.log("error", f"Bağımlılık temizleme hatası: {e}")mizlendi")
        except Exception as e:
            self.logger.log("error", f"Bağımlılık temizleme hatası: {e}")
...
# daha onceki versiyonlerin main satirlari
    ...
    if args.install:
        ...
        for package in args.install:
            ...
            if auto_importer.auto_install_package(package):
