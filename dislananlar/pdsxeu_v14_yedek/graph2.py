...
class GraphVertex:
    ...
    def __init__(self, value: Any, vertex_id: Optional[str] = None):
        ...
        self.features = np.zeros(1)  # GNN için özellik vektörü (varsayılan sıfır)

    def set_features(self, features: np.ndarray) -> None:
        """GNN için özellik vektörünü ayarlar."""
        self.features = features

    def __str__(self) -> str:
        return f"Vertex({self.value}, id={self.vertex_id}, features={self.features.shape})"
    ...
    def shortest_path(self, start_id: str, end_id: str) -> Dict:
        ...
        with self.lock:
            ...
            try:
                ...
                log.debug(f"En kısa yol bulundu (Dijkstra): start={start_id}, end={end_id}, length={result['length']}")
                return result
            except Exception as e:
                log.error(f"En kısa yol bulma hatası: {str(e)}")
                raise PdsXException(f"En kısa yol bulma hatası: {str(e)}")

    def bellman_ford(self, start_id: str) -> Dict:
        """Bellman-Ford algoritması ile en kısa yolları bulur (negatif ağırlıklar için)."""
        with self.lock:
            ...
            try:
                ...
                # Tüm kenarları V-1 kez gevşet
                for _ in range(self.vertex_count - 1):
                    for source_id in self.edges:
                        for target_id, weight in self.edges[source_id]:
                            if distances[source_id] != float('inf') and distances[source_id] + weight < distances[target_id]:
                ...
                for source_id in self.edges:
                    for target_id, weight in self.edges[source_id]:
                        if distances[source_id] != float('inf') and distances[source_id] + weight < distances[target_id]:
                            raise PdsXException("Grafikte negatif ağırlıklı döngü tespit edildi")
                
                result = {
                    "distances": {vid: dist if dist != float('inf') else -1 for vid, dist in distances.items()},
                    "predecessors": {vid: pred for vid, pred in predecessors.items()}
                }
                log.debug(f"Bellman-Ford tamamlandı: start={start_id}")
                return result
            except Exception as e:
                log.error(f"Bellman-Ford hatası: {str(e)}")
                raise PdsXException(f"Bellman-Ford hatası: {str(e)}")

    def floyd_warshall(self) -> Dict:
        """Floyd-Warshall algoritması ile tüm düğüm çiftleri arasındaki en kısa yolları bulur."""
        with self.lock:
            try:
                n = self.vertex_count
                ...
                # Kenar ağırlıklarını doldur
                for source_id in self.edges:
                    for target_id, weight in self.edges[source_id]:
                        dist[source_id][target_id] = weight
                        next_vertex[source_id][target_id] = target_id
                ...
                # Yollar için yardımcı fonksiyon
                ...
                result = {
                    "distances": {vid1: {vid2: dist[vid1][vid2] if dist[vid1][vid2] != float('inf') else -1 
                                         for vid2 in self.vertices} for vid1 in self.vertices},
                    "paths": {vid1: {vid2: get_path(vid1, vid2) for vid2 in self.vertices} 
                              for vid1 in self.vertices}
                }
                log.debug("Floyd-Warshall tamamlandı")
                return result
            except Exception as e:
                log.error(f"Floyd-Warshall hatası: {str(e)}")
                raise PdsXException(f"Floyd-Warshall hatası: {str(e)}")

    def gnn_layer(self, feature_dim: int, aggregation: str = "SUM", weights: Optional[np.ndarray] = None) -> Dict:
        """
        Basit bir Graph Neural Network katmanı uygular (mesaj geçişi ve toplama).
        Her düğüm için komşu özelliklerini toplar ve günceller.
        """
        with self.lock:
            try:
                aggregation = aggregation.upper()
                if aggregation not in ("SUM", "MEAN", "MAX"):
                    raise PdsXException(f"Desteklenmeyen toplama yöntemi: {aggregation}")
                
                # Özellik vektörlerini başlat (varsayılan sıfır)
                for vertex in self.vertices.values():
                    if vertex.features.shape[0] != feature_dim:
                        vertex.features = np.zeros(feature_dim)
                
                # Ağırlık matrisi (varsayılan kimlik matrisi)
                if weights is None:
                    weights = np.eye(feature_dim)
                elif weights.shape != (feature_dim, feature_dim):
                    raise PdsXException(f"Geçersiz ağırlık matrisi boyutu: {weights.shape}")
                
                # Mesaj geçişi ve toplama
                new_features = {}
                for vertex_id in self.vertices:
                    neighbors = [self.vertices[tid].features for tid, _ in self.edges[vertex_id]]
                    if neighbors:
                        if aggregation == "SUM":
                            aggregated = np.sum(neighbors, axis=0)
                        elif aggregation == "MEAN":
                            aggregated = np.mean(neighbors, axis=0)
                        elif aggregation == "MAX":
                            aggregated = np.max(neighbors, axis=0)
                    else:
                        aggregated = np.zeros(feature_dim)
                    
                    # Özellik güncelleme: W * (kendi özellik + toplanan özellik)
                    new_features[vertex_id] = np.dot(weights, self.vertices[vertex_id].features + aggregated)
                
                # Özellikleri güncelle
                for vertex_id, features in new_features.items():
                    self.vertices[vertex_id].set_features(features)
                
                result = {
                    "features": {vid: v.features.tolist() for vid, v in self.vertices.items()}
                }
                log.debug(f"GNN katmanı uygulandı: feature_dim={feature_dim}, aggregation={aggregation}")
                return result
            except Exception as e:
                log.error(f"GNN katmanı hatası: {str(e)}")
                raise PdsXException(f"GNN katmanı hatası: {str(e)}")
    ...
    def minimum_spanning_tree(self) -> List[Dict]:
        ...
        with self.lock:
            try:
                ...
                for source_id in self.edges:
                    for target_id, weight in self.edges[source_id]:
                        if self.directed or source_id < target_id:
        ...
        with self.lock:
            try:
                ...
                for vertex_id, vertex in self.vertices.items():
                    node_label = f"{vertex.value}\nID: {vertex_id}\nFeatures: {vertex.features.tolist()}"
...
class GraphManager:
    ...
    def parse_graph_command(self, command: str) -> None:
        ...
        try:
            ...
            elif command_upper.startswith("GRAPH BELLMAN FORD "):
                match = re.match(r"GRAPH BELLMAN FORD\s+(\w+)\s+(\w+)\s+(\w+)", command, re.IGNORECASE)
                if match:
                    graph_id, start_id, var_name = match.groups()
                    graph = self.graphs.get(graph_id)
                    if not graph:
                        raise PdsXException(f"Grafik bulunamadı: {graph_id}")
                    result = graph.bellman_ford(start_id)
                    self.interpreter.current_scope()[var_name] = result
                else:
                    raise PdsXException("GRAPH BELLMAN FORD komutunda sözdizimi hatası")
            elif command_upper.startswith("GRAPH FLOYD WARSHALL "):
                match = re.match(r"GRAPH FLOYD WARSHALL\s+(\w+)\s+(\w+)", command, re.IGNORECASE)
                if match:
                    ...
                    raise PdsXException("GRAPH FLOYD WARSHALL komutunda sözdizimi hatası")
            elif command_upper.startswith("GRAPH GNN "):
                match = re.match(r"GRAPH GNN\s+(\w+)\s+(\d+)\s+(\w+)\s*(\[.+\])?\s+(\w+)", command, re.IGNORECASE)
                if match:
                    graph_id, feature_dim, aggregation, weights_str, var_name = match.groups()
                    graph = self.graphs.get(graph_id)
                    if not graph:
                        raise PdsXException(f"Grafik bulunamadı: {graph_id}")
                    feature_dim = int(feature_dim)
                    weights = np.array(eval(weights_str)) if weights_str else None
                    result = graph.gnn_layer(feature_dim, aggregation, weights)
                    self.interpreter.current_scope()[var_name] = result
                else:
                    raise PdsXException("GRAPH GNN komutunda sözdizimi hatası")
