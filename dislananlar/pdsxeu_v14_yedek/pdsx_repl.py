...
try:
    ...
    print(f"⚠️  AutoImporter import hatası: {e}")
    ...
    try:
        ...
        print("🔧 PDS-X AutoImporter REPL desteği yüklendi")
        AUTO_IMPORTER_AVAILABLE = True
        SilentAutoImporter = None
        auto_setup_for_repl = None
        check_requirements_status = None
    except ImportError:
        print("⚠️  AutoImporter bulunamadı, temel REPL modu")
        AUTO_IMPORTER_AVAILABLE = False
        AutoImporter = None
        SilentAutoImporter = None
        setup_pdsX_environment = None
        auto_setup_for_repl = None
        check_requirements_status = None
...
class PDSXREPLSession:
    ...
    def __init__(self, auto_importer=None):
        ...
        if AUTO_IMPORTER_AVAILABLE and SilentAutoImporter:
    ...
    def execute_repl_command(self, command: str) -> bool:
        ...
        return None  # Normal PDS-X komutu
...
def start_pdsx_repl(auto_importer=None):
    ...
    if auto_importer is None and AUTO_IMPORTER_AVAILABLE:
        if SilentAutoImporter:
            # Sessiz otomatik kurulum
            print("📦 Sessiz otomatik kurulum modu aktif")
        elif 'AutoImporter' in globals() and AutoImporter:
            try:
