...
from pdsx_exception2 import ExceptionManager # ExceptionManager eklendi
from oop_and_class2 import OOPManager # OOPManager eklendi
from multithreading_process import MultithreadingProcessManager # MultithreadingProcessManager eklendi
from memory_manager import MemoryManager # MemoryManager eklendi
...
# ExceptionManager sınıfı pdsx_exception2.py'den geliyor
...
class PDSXIntegrator:
    ...
    def init_core_components(self):
        ...
        self.core = Core()
        self.memory_manager = MemoryManager(self) # Interpreter nesnesini buraya ilettim
    ...
    def init_infrastructure(self):
        ...
        from pdsx_exception2 import PdsXException, ExceptionManager # Güncel ExceptionManager kullanıldı
        from f11_backtrace_logger import BacktraceLogger
        from f12_timer_manager import TimerManager
        
        self.event_system = EventSystem()
        self.exception_handler = ExceptionManager(self) # interpreter argümanı verildi
        self.logger = BacktraceLogger()
        self.timer = TimerManager()
        
        # Bağımlılıkları ayarla
        self.exception_handler.set_logger(self.logger) # ExceptionManager'da set_logger metodu yok, kaldırıldı
...
class PDSXv14uInterpreter:
    def __init__(self):
        ...
        self.sqlite_manager = SQLiteManager(self) # SQLiteManager eklendi
        ...
        self.memory_manager = MemoryManager(self) # MemoryManager eklendi
        self.pipe_manager = PipeManager(self) # PipeManager eklendi
        self.tree_manager = TreeManager(self) # TreeManager eklendi
        self.graph_manager = GraphManager(self)
        self.functional_manager = FunctionalManager(self)
        self.save_manager = SaveManager(self) # SaveManager eklendi
        self.backtrace_logger = BacktraceLogger(self)
        self.timer_manager = TimerManager(self.event_manager)
        self.repl_extensions = ReplyExtension(self) # ReplyExtension eklendi
        self.data_structures = DataStructures(self)
        self.oop_manager = OOPManager(self) # OOPManager eklendi
        self.save_load_system = SaveLoadSystem(self) # SaveLoadSystem eklendi
        self.multithreading_manager = MultithreadingProcessManager(self) # MultithreadingProcessManager eklendi
        self.module_manager = ModuleManager(os.path.dirname(os.path.abspath(__file__)))
        self.database_isam = DatabaseManager(self)
        self.pipe_monitor_gui = PipeMonitorGUIManager(self) # PipeMonitorGUIManager eklendi
        self.report_exporter = ExportReportDocManager(self)
        self.plugin_manager = PluginManager()
        self.exception_manager = ExceptionManager(self) # ExceptionManager eklendi
        ...
        # Tip Tablosu (MemoryManager'dan alınmalı veya burada güncellenmeli)
        self.type_table = {
    ...
    def evaluate_expression(self, expr, scope_name=None):
        # NOTE: Bu expr_cache burada tanımlanmamış, eğer kullanılacaksa sınıf seviyesinde tanımlanmalı.
        # Örneğin: self.expr_cache = {}
        # Geçici olarak burada tanımlıyorum, ancak gerçek bir projede bu sınıfın __init__ metoduna taşınmalı.
        if not hasattr(self, 'expr_cache'):
            self.expr_cache = {}
    ...
    def parse_program(self, code, module_name="main", lightweight=False, as_library=False):
        ...
        while i < len(lines):
            ...
            if line_upper.startswith("END CLASS"):
                if current_class:
                    # OOPManager'ın build_class metodunu çağırmadan önce ClassDefinition'ı tanımlamamız gerekiyor
                    # Ancak bu yapı, ClassDefinition'ın nerede tanımlandığına bağlı.
                    # oop_and_class2.py'de ClassDefinition mevcut, bu yüzden doğrudan kullanabiliriz.
                    self.classes[current_class] = self.oop_manager.build_class_from_info(class_info[current_class])
            ...
            if current_class:
                if line_upper.startswith("SUB ") or line_upper.startswith("PRIVATE SUB ") or \
                   line_upper.startswith("FUNCTION ") or line_upper.startswith("PRIVATE FUNCTION "):
                    ...
                    if match:
                        ...
                        # Bu lambda fonksiyonu, ClassDefinition'daki execute_method'u doğru şekilde çağıracak şekilde ayarlanmalı
                        # Şu anki haliyle genel bir lambda, ClassDefinition'ın metot çağrı mekanizmasına uymayabilir.
                        method_lambda = lambda self_instance, *args, **kwargs: self_instance.execute_method(method_name, method_body, params, args, scope_name=current_class)
                        if is_private:
                ...
                if line_upper.startswith("STATIC "):
                    ...
                    if match:
                        ...
                        i += 1
                        continue
            if line_upper.startswith("TYPE "): # MemoryManager'ın parse_memory_command'ına yönlendirildi
                self.memory_manager.parse_memory_command(line)
                i += 1
                while i < len(lines) and not lines[i].strip().upper().startswith("END TYPE"):
                    self.memory_manager.parse_memory_command(lines[i].strip()) # Type içindeki FIELD satırlarını da işle
                    i += 1
                i += 1 # END TYPE satırını geç
                continue
            if line_upper.startswith("ENUM "): # MemoryManager'ın parse_memory_command'ına yönlendirildi
                self.memory_manager.parse_memory_command(line)
                i += 1
                while i < len(lines) and not lines[i].strip().upper().startswith("END ENUM"):
                    self.memory_manager.parse_memory_command(lines[i].strip()) # Enum içindeki değer satırlarını da işle
                    i += 1
                i += 1 # END ENUM satırını geç
                continue
    ...
    def execute_command(self, command, scope_name=None):
        if isinstance(command, dict):
            ...
            if cmd_type == "query":
                ...
                return None
            # TYPE ve ENUM komutları MemoryManager tarafından işlenecek
            # if cmd_type == "type":
        ...
        try:
            ...
            if command_upper.startswith("DIM"):
                ...
                if match:
                    ...
                    if dims:
                        ...
                        elif var_type.upper() in self.memory_manager.type_manager.types: # MemoryManager'ın type_manager'ından kontrol et
                            type_info = self.memory_manager.type_manager.types[var_type.upper()]
                            if type_info["kind"] == "STRUCT":
                                self.current_scope()[var_name] = self.memory_manager.type_manager.create_struct_instance(var_type.upper()) # Struct oluştur
                            elif type_info["kind"] == "UNION":
                                # Union için özel oluşturma veya varsayılan değer
                                self.current_scope()[var_name] = None # Placeholder
                            elif type_info["kind"] == "ENUM":
                                self.current_scope()[var_name] = self.memory_manager.type_manager.create_enum_instance(var_type.upper(), list(type_info["values"].keys())[0]) # Enum oluştur
                            elif type_info["kind"] == "CLASS":
                                self.current_scope()[var_name] = self.memory_manager.type_manager.create_class_instance(var_type.upper()) # Class oluştur
                        else:
                        ...
                        if var_type.upper() in self.type_table:
                            ...
                            type_info = self.memory_manager.type_manager.types[var_type.upper()]
                            if type_info["kind"] == "STRUCT":
                                self.current_scope()[var_name] = self.memory_manager.type_manager.create_struct_instance(var_type.upper())
                            elif type_info["kind"] == "UNION":
                                self.current_scope()[var_name] = None # Placeholder
                            elif type_info["kind"] == "ENUM":
                                self.current_scope()[var_name] = self.memory_manager.type_manager.create_enum_instance(var_type.upper(), list(type_info["values"].keys())[0])
                            elif type_info["kind"] == "CLASS":
                                self.current_scope()[var_name] = self.memory_manager.type_manager.create_class_instance(var_type.upper())
                        else:
                            raise PdsXException(f"Geçersiz veri tipi: {var_type}")
                    return None
            ...
            if command_upper.startswith("SQLITE"): # SQLiteManager komutları
                self.sqlite_manager.parse_sqlite_command(command)
                return None
            ...
            if command_upper.startswith("TRY"): # ExceptionManager'ın TRY CATCH'i kullanılıyor
                self.exception_manager.parse_exception_command(command)
                return None
            ...
            if command_upper.startswith("THREAD"):
                # MultithreadingProcessManager'ın THREAD CREATE'i kullanılıyor
                self.multithreading_manager.parse_multithreading_command(command)
                return None
            if command_upper.startswith("PIPE "): # PipeManager komutları
            ...
            if command_upper.startswith("SAVE PIPE"):
                self.save_manager.parse_save_command(command) # save.py'daki SaveManager
                return None
            ...
            if command_upper.startswith("LOAD MODULE "):
                match = re.match(r"LOAD MODULE\s+(\S+)(?:\s+AS\s+(\w+))?", command, re.IGNORECASE)
                if match:
                    module_name, alias = match.groups()
                    module = self.module_manager.load_module(module_name, alias)
                    # Modül yüklendikten sonra sınıfları, fonksiyonları ve değişkenleri tanıt
                    if hasattr(module, "__pdsX_exports__"):
                        # Sınıfları tanıt
                        for class_name, class_obj in module.__pdsX_exports__.get("classes", {}).items():
                            self.classes[class_name] = class_obj
                            self.modules[self.current_module]["classes"][class_name] = class_obj
                        # Fonksiyonları tanıt 
                        for func_name, func_obj in module.__pdsX_exports__.get("functions", {}).items():
                            self.function_table[func_name] = func_obj
                        # Global değişkenleri tanıt
                        for var_name, var_value in module.__pdsX_exports__.get("variables", {}).items():
                            self.global_vars[var_name] = var_value
                    
                    if alias:
                        self.current_scope()[alias] = module
                    else:
                        self.current_scope()[module_name] = module
                    print(f"[PDS-X] {module_name} modülü yüklendi")
                    return None
                raise PdsXException("LOAD MODULE komutunda sözdizimi hatası")
                
            if command_upper.startswith("UNLOAD MODULE "):
                match = re.match(r"UNLOAD MODULE\s+(\w+)", command, re.IGNORECASE)
                if match:
                    module_name = match.group(1)
                    self.module_manager.unload_module(module_name)
                    if module_name in self.current_scope():
                        del self.current_scope()[module_name]
                    return None
                raise PdsXException("UNLOAD MODULE komutunda sözdizimi hatası")
                
            if command_upper == "LIST MODULES":
                modules = self.module_manager.list_modules()
                if not modules:
                    print("Yüklü modül yok")
                else:
                    print("\nYüklü Modüller:")
                    print("-" * 60)
                    for mod in modules:
                        alias_str = f" (takma ad: {mod['alias']})" if mod['alias'] else ""
                        print(f"{mod['name']}{alias_str}")
                        print(f"  Konum: {mod['path']}")
                    print("-" * 60)
                return None
                
            if command_upper.startswith("SAVE MODULE "):
                match = re.match(r"SAVE MODULE\s+(\w+)(?:\s+AS\s+\"([^\"]+)\")?", command, re.IGNORECASE)
                if match:
                    module_name, file_path = match.groups()
                    saved_path = self.module_manager.save_module(module_name, file_path)
                    print(f"Modül kaydedildi: {saved_path}")
                    return None
                raise PdsXException("SAVE MODULE komutunda sözdizimi hatası")
                
            if command_upper.startswith("EDIT MODULE "):
                match = re.match(r"EDIT MODULE\s+(\w+)", command, re.IGNORECASE)
                if match:
                    module_name = match.group(1)
                    source_path = self.module_manager.edit_module(module_name)
                    print(f"Modül düzenleme için hazır: {source_path}")
                    return None
                raise PdsXException("EDIT MODULE komutunda sözdizimi hatası")
            
            # Save/Load System komutları (save_load_system2.py tarafından işlenir)
            if command_upper.startswith(("SAVE DATA ", "LOAD DATA ", "SAVE ASYNC DATA ", "LOAD ASYNC DATA ",
                                       "SAVE DISTRIBUTED ", "LOAD DISTRIBUTED ", "SAVE STREAM ", "LOAD STREAM ",
                                       "SAVE ENCRYPT ", "READ DATA ", "WRITE DATA ", "DATA ", "READ ",
                                       "ANALYZE DATA ", "VISUALIZE DATA ", "QUANTUM DATA ", "HOLO DATA ",
                                       "SMART DATA ", "TEMPORAL DATA ", "PREDICT DATA ")):
                self.save_load_system.parse_save_load_command(command)
                return None

            # Tree Manager komutları (tree3.py tarafından işlenir)
            if command_upper.startswith(("TREE CREATE ", "TREE ADD ", "TREE REMOVE ", "TREE TRAVERSE ",
                                       "TREE SEARCH ", "TREE SEARCH OPTIMIZED ", "TREE PATH SUM ", 
                                       "TREE SHORTEST PATH ", "TREE DIAMETER ", "TREE HEIGHT ", "TREE VISUALIZE ")):
                self.tree_manager.parse_tree_command(command)
                return None

            # SQLite Manager komutları (sqlite.py tarafından işlenir)
            if command_upper.startswith(("SQLITE CONNECT ", "SQLITE EXECUTE ", "SQLITE QUERY ", 
                                       "SQLITE BEGIN TRANSACTION ", "SQLITE COMMIT ", "SQLITE ROLLBACK ",
                                       "SQLITE CLOSE ", "SQLITE VACUUM ", "SQLITE BACKUP ", "SQLITE SCHEMA ")):
                self.sqlite_manager.parse_sqlite_command(command)
                return None

            # Save Manager komutları (ayrıca SaveLoadSystem içinde de var, burada genel olanlar)
            if command_upper.startswith(("SAVE DATA ", "LOAD DATA ", "SAVE SERIALIZE ", "SAVE COMPRESS ",
                                       "SAVE ENCRYPT ", "SAVE VERSION ", "SAVE DISTRIBUTED ", 
                                       "LOAD DISTRIBUTED ", "SAVE DIFF ", "LOAD DIFF ", "SAVE PROVENANCE ",
                                       "SAVE HOLO ", "LOAD HOLO ", "SAVE QUANTUM ", "LOAD QUANTUM ",
                                       "SAVE HEALING ", "LOAD HEALING ")):
                # SaveLoadSystem tarafından işlendiği için burada tekrar işlemeye gerek yok
                # self.save_manager.parse_save_command(command)
                pass

            # Reply Extension komutları (reply_extension.py tarafından işlenir)
            if command_upper.startswith(("REPLY SEND ", "REPLY ASYNC ", "REPLY DISTRIBUTED ",
                                       "REPLY WEBSOCKET ", "REPLY ENCRYPT ", "REPLY ANALYZE ",
                                       "REPLY VISUALIZE ", "REPLY QUANTUM ", "REPLY HOLO ",
                                       "REPLY SMART ", "REPLY TEMPORAL ", "REPLY PREDICT ")):
                self.repl_extensions.parse_reply_command(command)
                return None

            # Pipe Monitor GUI Manager komutları (pipe_monitor_gui.py tarafından işlenir)
            if command_upper.startswith(("PIPE CREATE ", "PIPE WRITE ", "PIPE READ ", "PIPE ASYNC WRITE ",
                                       "PIPE ASYNC READ ", "MONITOR START ", "MONITOR STOP ", "GUI CREATE ",
                                       "GUI ADD WIDGET ", "GUI SHOW ", "GUI PLOT ", "PIPE ANALYZE ",
                                       "PIPE VISUALIZE ", "PIPE QUANTUM ", "PIPE HOLO ", "PIPE SMART ",
                                       "PIPE TEMPORAL ", "PIPE PREDICT ")):
                self.pipe_monitor_gui.parse_pipe_monitor_gui_command(command)
                return None

            # Exception Manager komutları (pdsx_exception2.py tarafından işlenir)
            if command_upper.startswith(("ERROR ", "MONERROR ", "ANALYZEERROR ", "VISUALIZEERROR ",
                                       "SUPPRESSERROR ", "UNSUPPRESSERROR ", "CATCHERROR ", "TRACE ON",
                                       "TRACE OFF", "DEBUG ON", "DEBUG OFF", "CLEARERROR", "TRY ",
                                       "ON ERROR GOTO ", "REGISTER ERROR HANDLER ", "UNREGISTER ERROR HANDLER ")):
                self.exception_manager.parse_exception_command(command)
                return None

            # OOP Manager komutları (oop_and_class2.py tarafından işlenir)
            if command_upper.startswith(("CLASS DEFINE ", "CLASS ASYNC DEFINE ", "CLASS METHOD ", "CLASS PROPERTY ",
                                       "CLASS INSTANTIATE ", "CLASS CALL ", "CLASS SET PROPERTY ", "CLASS GET PROPERTY ",
                                       "STRUCT DEFINE ", "UNION DEFINE ", "STRUCT SET FIELD ", "STRUCT GET FIELD ",
                                       "UNION SET FIELD ", "UNION GET FIELD ", "POINTER CREATE ", "POINTER SET ",
                                       "POINTER DEREFERENCE ", "STREAM CREATE ", "STREAM CONNECT ", "STREAM WRITE ",
                                       "STREAM READ ", "STREAM PEEK ", "STREAM RESET ", "STREAM CLOSE ",
                                       "CLASS SERIALIZE ", "CLASS ENCRYPT ", "CLASS ANALYZE ", "CLASS VISUALIZE ",
                                       "CLASS QUANTUM ", "CLASS HOLO ", "CLASS SMART ", "CLASS TEMPORAL ",
                                       "CLASS PREDICT ")):
                self.oop_manager.parse_oop_command(command)
                return None

            # Multithreading Process Manager komutları (multithreading_process.py tarafından işlenir)
            if command_upper.startswith(("THREAD CREATE ", "PROCESS CREATE ", "THREAD JOIN ", "PROCESS JOIN ",
                                       "TASK POOL ", "PARALLEL MAP ", "ASYNC EXECUTE ", "QUEUE CREATE ",
                                       "QUEUE PUT ", "QUEUE GET ", "LOCK CREATE ", "LOCK ACQUIRE ",
                                       "LOCK RELEASE ", "EVENT CREATE ", "EVENT SET ", "EVENT WAIT ",
                                       "QUANTUM TASK ", "HOLO TASK ", "SMART TASK ", "TEMPORAL TASK ",
                                       "PREDICT TASK ", "ANALYZE TASK ", "VISUALIZE TASK ")):
                self.multithreading_manager.parse_multithreading_command(command)
                return None

            # Memory Manager komutları (memory_manager.py tarafından işlenir)
            if command_upper.startswith(("NEW ", "DELETE ", "SIZEOF ", "TYPE ", "ENUM ")):
                self.memory_manager.parse_memory_command(command)
                return None
    ...
    async def execute_command_async(self, command, scope_name=None):
        try:
            # REPLY komutları reply_extension ile
            if isinstance(command, str) and command.strip().upper().startswith("REPLY"):
                self.repl_extensions.parse_reply_command(command)
                return None
            # SAVE_LOAD_SYSTEM komutları
            if isinstance(command, str) and command.strip().upper().startswith(("SAVE DATA ", "LOAD DATA ", "SAVE ASYNC DATA ", "LOAD ASYNC DATA ",
                                                                               "SAVE DISTRIBUTED ", "LOAD DISTRIBUTED ", "SAVE STREAM ", "LOAD STREAM ",
                                                                               "SAVE ENCRYPT ", "READ DATA ", "WRITE DATA ", "DATA ", "READ ",
                                                                               "ANALYZE DATA ", "VISUALIZE DATA ", "QUANTUM DATA ", "HOLO DATA ",
                                                                               "SMART DATA ", "TEMPORAL DATA ", "PREDICT DATA "))):
                self.save_load_system.parse_save_load_command(command)
                return None
            # PIPE_MONITOR_GUI komutları
            if isinstance(command, str) and command.strip().upper().startswith(("PIPE CREATE ", "PIPE WRITE ", "PIPE READ ", "PIPE ASYNC WRITE ",
                                                                               "PIPE ASYNC READ ", "MONITOR START ", "MONITOR STOP ", "GUI CREATE ",
                                                                               "GUI ADD WIDGET ", "GUI SHOW ", "GUI PLOT ", "PIPE ANALYZE ",
                                                                               "PIPE VISUALIZE ", "PIPE QUANTUM ", "PIPE HOLO ", "PIPE SMART ",
                                                                               "PIPE TEMPORAL ", "PIPE PREDICT "))):
                self.pipe_monitor_gui.parse_pipe_monitor_gui_command(command)
                return None
            # Exception Manager komutları
            if isinstance(command, str) and command.strip().upper().startswith(("ERROR ", "MONERROR ", "ANALYZEERROR ", "VISUALIZEERROR ",
                                                                               "SUPPRESSERROR ", "UNSUPPRESSERROR ", "CATCHERROR ", "TRACE ON",
                                                                               "TRACE OFF", "DEBUG ON", "DEBUG OFF", "CLEARERROR", "TRY ",
                                                                               "ON ERROR GOTO ", "REGISTER ERROR HANDLER ", "UNREGISTER ERROR HANDLER "))):
                self.exception_manager.parse_exception_command(command)
                return None
            # OOP Manager komutları
            if isinstance(command, str) and command.strip().upper().startswith(("CLASS DEFINE ", "CLASS ASYNC DEFINE ", "CLASS METHOD ", "CLASS PROPERTY ",
                                                                               "CLASS INSTANTIATE ", "CLASS CALL ", "CLASS SET PROPERTY ", "CLASS GET PROPERTY ",
                                                                               "STRUCT DEFINE ", "UNION DEFINE ", "STRUCT SET FIELD ", "STRUCT GET FIELD ",
                                                                               "UNION SET FIELD ", "UNION GET FIELD ", "POINTER CREATE ", "POINTER SET ",
                                                                               "POINTER DEREFERENCE ", "STREAM CREATE ", "STREAM CONNECT ", "STREAM WRITE ",
                ...
                self.oop_manager.parse_oop_command(command)
                return None
            # Multithreading Process Manager komutları
            if isinstance(command, str) and command.strip().upper().startswith(("THREAD CREATE ", "PROCESS CREATE ", "THREAD JOIN ", "PROCESS JOIN ",
                                                                               "TASK POOL ", "PARALLEL MAP ", "ASYNC EXECUTE ", "QUEUE CREATE ",
                                                                               "QUEUE PUT ", "QUEUE GET ", "LOCK CREATE ", "LOCK ACQUIRE ",
                                                                               "LOCK RELEASE ", "EVENT CREATE ", "EVENT SET ", "EVENT WAIT ",
                                                                               "QUANTUM TASK ", "HOLO TASK ", "SMART TASK ", "TEMPORAL TASK ",
                ...
                self.multithreading_manager.parse_multithreading_command(command)
                return None
            # Memory Manager komutları
            if isinstance(command, str) and command.strip().upper().startswith(("NEW ", "DELETE ", "SIZEOF ", "TYPE ", "ENUM "))):
                self.memory_manager.parse_memory_command(command)
                return None
...
# Statik olarak yüklenen modüller
CORE_MODULES = [
    "pdsx_exception", # Zaten vardı
    "pdsx_exception2", # Yeni eklendi
    "module_manager", # Zaten vardı
    "core",
    "memory_manager", # Yeni eklendi
    "save_load_system2", # Zaten vardı
    "event",
    "libxcore",
    "auto_importer",
    "autoinstaller",
    "base_module_manager",
    "oop_and_class2", # Yeni eklendi
    "clazz",
    "save",
    "sqlite",
    "reply_extension",
    ...
    "f12_timer_manager",
    "multithreading_process", # Yeni eklendi
    "pipe_monitor_gui",
    "export_report_doc"
]

# Dinamik olarak yüklenebilecek modüller
LOADABLE_MODULES = [
    "core2-5",
    "database_sql_isam",
    "bytecode_engine",
    ...
    "tpl",
    "tpl3",
    "oop_and_class", # oop_and_class2 var, bu eski versiyon olabilir
    "eventx",
    "functional2",
    "graph2",
    "lib_db",
...
def validate_modules():
    """Modülleri doğrular ve hazırlar."""
    try:
        from module_validator import validate_all_modules # module_validator.py'den import edildi
    except ImportError:
        class MockModuleValidator:
            def validate_all_modules(self, modules):
                print(f"[PDS-X] Uyarı: 'module_validator' bulunamadı. Modül doğrulama atlanıyor.")
                return {m: True for m in modules}
        validate_all_modules = MockModuleValidator().validate_all_modules
    ...
    # Yüklenebilir modülleri kontrol et
...
async def main():
    ...
    required_libs = ["numpy", "pandas", "scipy", "aiohttp", "aiofiles", "pyyaml", "pdfplumber", 
                     "graphviz", "scikit-learn", "boto3", "websockets", "pycryptodome", 
                     "tkinter", "matplotlib", "psutil", "tensorflow"] # Yeni eklenen bağımlılıklar
    for lib in required_libs:
        install_missing_packages([lib]) # install_library yerine install_missing_packages kullanıldı
    ...
    if args.file:
        ...
        await interpreter.run_async() # Asenkron çalıştırma
...
if __name__ == "__main__":
    # Sadece ilk kez çağrıldığında bu kontrolü yapın
    if "pdsXuv14_initialized" not in globals():
        valid_modules = validate_modules()
        globals()["pdsXuv14_initialized"] = True # Bayrağı ayarla

     asyncio.run(main())
        compiled_code = self.bytecode_compiler.compile(code)
            result = await self.bytecode_manager.execute(compiled_code)
            self.metrics.record_execution_time(len(compiled_code))
            return result
        except Exception as e:
            log.error(f"[PdsX] Kod yürütme hatası: {str(e)}")
            raise PdsXException(f"Kod yürütme hatası: {str(e)}")   