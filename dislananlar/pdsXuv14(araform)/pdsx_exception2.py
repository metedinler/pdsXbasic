...
def _check_pdsx_env():
    ...
    if env_name and expected_env not in env_name and expected_env not in python_path:
        msg = f"UYARI: Şu anda '{env_name or python_path}' ortamındasınız. Doğru ortam: '{expected_env}'. Bağımlılık ve modül hataları oluşabilir."
        print(msg)
        log.warning(msg)
    elif not env_name and expected_env not in python_path:
        msg = f"UYARI: Sanal ortam algılanamadı. Aktif Python: {python_path}. Doğru ortam: '{expected_env}'."
        print(msg)
        log.warning(msg)
