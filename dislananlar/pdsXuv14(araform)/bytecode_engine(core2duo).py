...
# Cython için statik tipli yardımcı fonksiyon (kritik döngüler için)
if compiled:
...
class BytecodeEngine:
    """PDS-X BASIC v15 bayt kodu yürütme motoru, Intel Core Duo 2 ve CPython optimizasyonu ile."""
    def __init__(self, interpreter):
        ...
        self.metadata: Dict = {
            "bytecode_engine": {
                "version": "1.5.0",
                "dependencies": ["numpy", "pandas", "aiofiles", "cython"]
            }
        }
    ...
    async def op_simd_add(self, operands: List[Any]) -> None:
        ...
        if not isinstance(data, (np.ndarray, list)):
            raise PdsXTypeError(f"Geçersiz veri tipi: {data_var}")
    ...
    async def op_dim(self, operands: List[Any]) -> None:
        ...
        try:
            if initial_value:
                ...
                value = np.zeros(size, dtype=np.float32)  # SSE2 uyumlu
...
__pdsX_exports__ = {
    ...
    "variables": {
        "version": "1.5.0",
        "dependencies": ["numpy", "pandas", "pdsx_exception2"]
    }
}