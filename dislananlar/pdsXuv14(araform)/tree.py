...
class Tree:
        ...
        with self.lock:
            ...
            try:
                if self.tree_type == "BINARY":
                    ...
                    if isinstance(parent, RedBlackNode):
            ...
            try:
                ...
                def add_nodes_edges(node: TreeNode):
                    ...
                    if isinstance(node, RedBlackNode):
                        node_label += f"\nColor: {node.color}"
                        dot.node(node.node_id, node_label, color="red" if node.color == "RED" else "black", fontcolor="white" if node.color == "BLACK" else "black")
                    else:
    ...
    def _rebalance_redblack(self, node: RedBlackNode) -> None:
        """Kırmızı-siyah ağacı yeniden dengeler."""
        try:
            # Kırmızı-siyah ağaç dengeleme mantığı (basitleştirilmiş)
    ...
    def _rebalance_redblack_remove(self, node: RedBlackNode) -> None:
        ...
        # Basitleştirilmiş, gerçek uygulamada daha karmaşık
        log.debug("Kırmızı-siyah ağaç kaldırma sonrası dengelendi (basitleştirilmiş)")
...
__pdsX_exports__ = {
    ...
    "variables": {
        "version": "1.0.0",
        "dependencies": ["numpy", "pdsx_exception"]
    }
}