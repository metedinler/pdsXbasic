...
class EnvManager:
    ...
    def update_pip_if_needed(self, force_latest=False, silent=False):
        """Pip sürümünü günceller - Python 3.10 için uygun sürüm veya en son sürüm"""
        try:
            pip_cmd = str(self.venv_dir / ("Scripts" if os.name == "nt" else "bin") / "pip")
            log_level = "debug" if silent else "info"
            
            if force_latest:
                self.logger.log(log_level, "En son pip sürümü yükleniyor...")
                process = subprocess.run([pip_cmd, "install", "-U", "pip"], 
                                       check=True, capture_output=True, text=True)
                shutdown_manager.register_process(process)
            else:
                self.logger.log(log_level, "Python 3.10 için pip 21.2.4 yükleniyor...")
                process = subprocess.run([pip_cmd, "install", "pip==21.2.4"], 
                                       check=True, capture_output=True, text=True)
                shutdown_manager.register_process(process)
            ...
            if not silent:
                self.logger.log("info", "Pip güncelleme tamamlandı")
            return True
