...
class AutoImporter:
    ...
    def cleanup_on_shutdown(self):
        ...
        try:
            ...
            # Real-time monitoring durdur
            if hasattr(self, 'realtime_monitor') and self.realtime_monitor:
                try:
                    self.realtime_monitor.stop_monitoring()
                    self.logger.log("info", "Real-time monitoring durduruldu")
                except Exception as e:
                    self.logger.log("warning", f"Real-time monitoring durdurma hatası: {e}")
            
            # Terminal analyzer'ı kaydet
            if hasattr(self, 'terminal_analyzer') and self.terminal_analyzer:
                try:
                    self.terminal_analyzer.save_learned_dependencies()
                    self.logger.log("info", "Öğrenilmiş bağımlılıklar kaydedildi")
                except Exception as e:
                    self.logger.log("warning", f"Terminal analyzer kaydetme hatası: {e}")
    ...
    def run(self, packages: List[str]):
        try:
            ...
            # Terminal log analizi yap (önceki çalıştırmalardan öğrenilmiş bağımlılıklar)
            self.logger.log("info", "Terminal log analizi yapılıyor...")
            learned_packages = self.terminal_analyzer.analyze_terminal_logs()
            if learned_packages:
                self.logger.log("info", f"Log analizinde {len(learned_packages)} ek bağımlılık bulundu")
                packages.extend([pkg for pkg in learned_packages if pkg not in packages])
            ...
            for pkg in packages:
              ...
              # Kurulum sonrası log analizi (yeni öğrenilmiş bağımlılıklar)
            self.logger.log("info", "Kurulum sonrası log analizi yapılıyor...")
            post_install_packages = self.terminal_analyzer.analyze_terminal_logs()
            
            # Real-time monitor'dan tespit edilen hataları al
            detected_errors = self.realtime_monitor.get_detected_errors()
            auto_install_queue = self.realtime_monitor.get_auto_install_queue()
            
            # Tespit edilen hataları log'la
            if detected_errors:
                self.logger.log("info", f"Real-time monitoring {len(detected_errors)} hata tespit etti")
                for error in detected_errors:
                    self.logger.log("warning", f"Tespit edilen hata: {error.get('type', 'unknown')} - {error.get('package', 'unknown')}")
            
            if post_install_packages or auto_install_queue:
                additional_packages = list(set(post_install_packages + auto_install_queue))
                self.logger.log("info", f"Kurulum sonrası {len(additional_packages)} ek bağımlılık tespit edildi")
                
                # Ek paketleri yükle
                for pkg in additional_packages:
                    if pkg not in packages:  # Zaten yüklenmemiş ise
                        self.logger.log("info", f"Öğrenilmiş bağımlılık yükleniyor: {pkg}")
                        self.install_package(pkg, silent=True)
        ...
        finally:            # Monitoring'i her durumda durdur
            try:
                self.realtime_monitor.stop_monitoring()
            except Exception:
                pass
...
class RealTimeLogMonitor:
    ...
    def get_monitoring_stats(self) -> Dict:
        ...
        return {
            ...
            'auto_install_queue_count': len(self.auto_install_queue),            'thread_alive': self.monitor_thread.is_alive() if self.monitor_thread else False
        }
...
if __name__ == "__main__":
    ...
    def demo_auto_importer():
        ...
        try:
            ...
            detected = log_analyzer.analyze_log_content(sample_log_content)
    ...
    if args.analyze_log:
        ...
        if os.path.exists(args.analyze_log):
            detected = auto_importer.log_analyzer.analyze_log_file(args.analyze_log)
