...
# Version: 1.7.9.8 - Merged Features: Terminal Log Analysis, Argparse Replay, Enhanced Dependencies, Real-time Monitoring
# Date: June 25, 2025
...
#         try:
#             self.logger.log("info", "Gerekli paketler kontrol ediliyor ve yükleniyor...")
#             success_count = 0
#             total_count = len(REQUIRED_PACKAGES)
#             pip_cmd = str(self.venv_dir / "Scripts" / "pip.exe") if os.name == "nt" else str(self.venv_dir / "bin" / "pip")
#             for package_spec, import_name in REQUIRED_PACKAGES:
...
#             self.logger.log("info", f"Paket kurulum tamamlandı: {success_count}/{total_count} başarılı")
#         except Exception as e:
#             self.logger.log("error", f"Gerekli paketlerin kurulumu sırasında hata: {e}")

# auto_importer.py içinde EnvManager sınıfının güncellenmiş hali
...
class EnvManager:
    def __init__(self, venv_dir: Path = VENV_DIR, logger: AdvancedLogger = None):
        ...
        self.logger = logger if logger is not None else AdvancedLogger()
        self.python_path = None
        self.cache_manager = CacheManager(logger=self.logger)  # Önbellek yönetimi için

    def download_pip(self, target_version: str = "21.2.4") -> Optional[str]:
        """pip'i resmi kaynaktan indirir ve önbelleğe kaydeder."""
        try:
            pip_package = f"pip=={target_version}"
            pip_wheel_url = f"https://files.pythonhosted.org/packages/py3/p/pip/pip-{target_version}-py3-none-any.whl"
            cache_file = self.cache_manager.cache_dir / f"pip-{target_version}.whl"
            
            self.logger.log("info", f"pip {target_version} indiriliyor: {pip_wheel_url}")
            if not cache_file.exists():
                subprocess.run(
                    ["curl", "-o", str(cache_file), pip_wheel_url],
                    check=True, capture_output=True, text=True
                )
                with open(cache_file, "rb") as f:
                    file_hash = hashlib.sha256(f.read()).hexdigest()
                metadata = self.cache_manager.load_package_metadata()
                metadata[pip_package] = {
                    "version": target_version,
                    "timestamp": datetime.now().isoformat(),
                    "hash": file_hash
                }
                self.cache_manager.save_package_metadata(metadata)
                self.logger.log("info", f"pip {target_version} önbelleğe kaydedildi: {cache_file}")
            else:
                self.logger.log("info", f"pip {target_version} önbellekte bulundu: {cache_file}")
            return str(cache_file)
        except subprocess.CalledProcessError as e:
            self.logger.log("error", f"pip indirme hatası: {e.stderr}")
            return None
        except Exception as e:
            self.logger.log("error", f"pip indirme sırasında beklenmeyen hata: {e}")
            return None

    def break_and_reinstall_pip(self, pip_wheel_path: Optional[str] = None) -> bool:
        """Sanal ortamdaki mevcut pip kurulumunu kaldırır ve yeni bir pip sürümü kurar."""
        try:
            pip_cmd = str(self.venv_dir / "Scripts" / "pip.exe" if os.name == "nt" else self.venv_dir / "bin" / "pip")
            self.logger.log("info", "Mevcut pip kurulumu kaldırılıyor...")
            
            # Mevcut pip kurulumunu kaldırma
            if os.path.exists(pip_cmd):
                subprocess.run([pip_cmd, "uninstall", "pip", "-y"], capture_output=True, text=True)
                self.logger.log("info", "Mevcut pip kaldırıldı")
            else:
                self.logger.log("warning", "Mevcut pip bulunamadı, kaldırma işlemi atlandı")

            # Yeni pip sürümünü kurma
            if pip_wheel_path is None:
                pip_wheel_path = self.download_pip()
            if pip_wheel_path is None:
                self.logger.log("error", "pip wheel dosyası indirilemedi, kurulum başarısız")
                return False

            self.logger.log("info", f"Yeni pip kuruluyor: {pip_wheel_path}")
            venv_python = str(self.venv_dir / "Scripts" / "python.exe" if os.name == "nt" else self.venv_dir / "bin" / "python")
            result = subprocess.run(
                [venv_python, "-m", "pip", "install", pip_wheel_path, "--no-index"],
                check=True, capture_output=True, text=True
            )
            self.logger.log("info", f"pip başarıyla kuruldu: {result.stdout}")
            return True
        except subprocess.CalledProcessError as e:
            self.logger.log("error", f"pip yeniden kurulum hatası: {e.stderr}")
            return False
        except Exception as e:
            self.logger.log("error", f"pip yeniden kurulum sırasında beklenmeyen hata: {e}")
            return False

    def update_pip_if_needed(self, force_latest: bool = False, silent: bool = False) -> bool:
        """pip sürümünü kontrol eder ve gerekirse kırıp yeniden kurar."""
        try:
            pip_cmd = str(self.venv_dir / "Scripts" / "pip.exe" if os.name == "nt" else self.venv_dir / "bin" / "pip")
            if not os.path.exists(pip_cmd):
                self.logger.log("error", "pip komutu bulunamadı, yeniden kuruluyor...")
                return self.break_and_reinstall_pip()
            ...
            # pip sürümünün uyumluluğunu kontrol et
            target_version = "latest" if force_latest else "21.2.4"
            pip_package = "pip" if force_latest else f"pip=={target_version}"
            self.logger.log(log_level, f"pip hedef sürümü: {target_version}")

            # Uyumluluk testi: pip check ile bağımlılık çakışması kontrolü
            check_result = subprocess.run([pip_cmd, "check"], capture_output=True, text=True)
            if check_result.returncode != 0 or "no conflicts" not in check_result.stdout.lower():
                self.logger.log("warning", f"pip uyumluluk sorunu tespit edildi: {check_result.stderr}")
                return self.break_and_reinstall_pip()
            ...
            if result.returncode == 0:
                ...
                self.logger.log("warning", f"pip güncelleme başarısız, yeniden kurulum deneniyor: {result.stderr}")
                return self.break_and_reinstall_pip()
        except Exception as e:
            self.logger.log("error", f"pip güncelleme hatası, yeniden kurulum deneniyor: {e}")
            return self.break_and_reinstall_pip()

    def setup_environment(self) -> bool:
        """Sanal ortamı hazırlar, pip'i indirir ve kurar."""
        try:
            self.logger.log("info", "Ortam hazırlanıyor...")
            if self.is_running_in_venv():
                self.logger.log("info", "Zaten sanal ortamda çalışıyoruz.")
            ...
            # pip'i indir ve kur
            if not self.update_pip_if_needed(force_latest=False, silent=False):
                self.logger.log("warning", "pip güncelleme başarısız, kırma ve yeniden kurulum deneniyor...")
                if not self.break_and_reinstall_pip():
                    self.logger.log("error", "pip kurulumu başarısız, devam edilemiyor")
                    return False
...
class RealTimeLogMonitor:
    ...
    def _trigger_emergency_install(self, package_name: str):
        ...
        try:
            # Anlık kurulum yerine, kuyruğa eklenmiş paketi AutoImporter üzerinden kur
            self.logger.log("info", f"Acil kurulum tetikleniyor: {package_name}")
            # AutoImporter'ın install_package metodunu çağır
            success = self.auto_install_package(package_name)
            if success:
                self.logger.log("info", f"{package_name} acil kurulum başarılı")
            else:
                self.logger.log("error", f"{package_name} acil kurulum başarısız")
        except Exception as e:
...
# Terminal Log Analizi ve Bağımlılık Öğrenme Sistemi (Devam)
class TerminalLogAnalyzer:
        ...
        try:
            if self.learned_dependencies_file.exists():
                with open(self.learned_dependencies_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    self.logger.log("info", "Öğrenilmiş bağımlılıklar yüklendi")
                    return data
            return {
                "discovered_packages": [],
                "version_conflicts": [],
            ...
            self.logger.log("error", f"Öğrenilmiş bağımlılıklar yükleme hatası: {e}")
            return {}
    ...
    def save_learned_dependencies(self):
        ...
        try:
            ...
            self.logger.log("info", "Öğrenilmiş bağımlılıklar kaydedildi")
        except Exception as e:
            self.logger.log("error", f"Öğrenilmiş bağımlılıklar kaydetme hatası: {e}")

    # package_to_module ve builtin_modules tanımları
    @property
    def package_to_module(self) -> Dict[str, str]:
        """Paket adlarını modül adlarına eşleştiren sözlük"""
        return {
            'opencv-python': 'cv2',
            'pillow': 'PIL',
            'pyyaml': 'yaml',
    ...
    @property
    def builtin_modules(self) -> List[str]:
        """Python'un yerleşik modülleri"""
        return [
            'tkinter', 'sqlite3', 'urllib', 'json', 'os', 'sys', 'time',
            'datetime', 're', 'threading', 'subprocess', 'hashlib', 'logging', 'argparse'
        ]

# AutoImporter Sınıfı (Devam)
class AutoImporter:
    def check_stop_combination(self, event):
        """Ctrl+Shift+Q kombinasyonunu kontrol eder"""
        try:
            keyboard = get_keyboard()
            if keyboard:
                if keyboard.is_pressed('ctrl') and keyboard.is_pressed('shift') and event.name == 'q':
                    self.logger.log("info", "Ctrl+Shift+Q kombinasyonu algılandı, acil kapatma başlatılıyor")
                    shutdown_manager.emergency_shutdown()
        except Exception as e:
            self.logger.log("error", f"Klavye kombinasyonu kontrol hatası: {e}")

    def cleanup_on_shutdown(self):
        """Program kapanırken temizlik işlemlerini yapar"""
        try:
            self.logger.log("info", "AutoImporter kapanış temizliği başlatılıyor")
            # Aktif izlemeyi durdur
            if hasattr(self, 'realtime_monitor') and self.realtime_monitor.is_monitoring():
                self.realtime_monitor.stop_monitoring()
            # Kuyrukları temizle
            if hasattr(self, 'terminal_analyzer'):
                self.terminal_analyzer.clear_detected_errors()
            # Logları kaydet
            self.logger.log("info", "Kapanış temizliği tamamlandı")
        except Exception as e:
            self.logger.log("error", f"Kapanış temizliği hatası: {e}")

    def check_build_tools(self):
        """Derleme araçlarını kontrol eder"""
        return check_build_tools(self.logger)
    ...
    async def async_install_package(self, package: str) -> bool:
        """Asenkron paket kurulumu"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, lambda: self.install_package(package, silent=True))
