...
class PipOutputAnalyzer:
    ...
    def analyze_failure(self, output: str) -> Dict[str, Any]:
        self.logger.log("debug", "Pip hata çıktısı analizi başlatıldı.")
        for key, pattern in self.error_patterns.items():
            match = pattern.search(output)
            if match:
                self.logger.log("warning", f"Hata deseni bulundu: {key}")
                
                details = {"raw": match.group(0)}
                
                if key == "dependency_conflict":
                    # Çakışma detaylarını ayrıştırmaya çalış
                    conflict_details_pattern = re.compile(
                        r"The conflict is caused by:(.*?)(?:\n\nTo fix this|\Z)", 
                        re.DOTALL | re.IGNORECASE
                    )
                    details_match = conflict_details_pattern.search(output)
                    
                    if details_match:
                        conflict_text = details_match.group(1).strip()
                        # Örnek: "requests 2.28.1 has requirement charset-normalizer<3,>=2, but you have..."
                        # Örnek: "packageA 1.0 depends on packageC>=3.0"
                        dependency_lines_pattern = re.compile(
                            r"([\w\-\.]+)\s+([\w\.\-]+)\s+(?:depends on|has requirement)\s+(.+)", 
                            re.IGNORECASE
                        )
                        ...
                        for line in conflict_text.split('\n'):
                            ...
                            line_match = dependency_lines_pattern.match(line)
                            if line_match:
                                conflicts.append({
                                    "source_package": line_match.group(1),
                                    "source_version": line_match.group(2),
                                    "conflicting_dependency": line_match.group(3)
                                })
                            else:
                                conflicts.append({"raw_line": line})
                        details["conflicts"] = conflicts
                
                # no_matching_distribution için paketi yakala
                if key == "no_matching_distribution":
                    package_match = re.search(r"satisfies the requirement (.+?)(?: from|$)", match.group(0), re.IGNORECASE)
                    if package_match:
                        details["package"] = package_match.group(1).strip()

                return {"error_type": key, "details": details}
                
        return {"error_type": "unknown", "details": {"raw": "Bilinmeyen bir pip hatası."}}
...
class ConflictResolver:
    ...
    def suggest_resolution(self, package: str, failure_details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        details = failure_details.get("details", {})

        if error_type == "dependency_conflict":
            conflicts = details.get("conflicts")
            if conflicts:
                self.logger.log("error", "Bağımlılık çakışması tespit edildi:")
                for conflict in conflicts:
                    if 'raw_line' in conflict:
                        self.logger.log("error", f"  - {conflict['raw_line']}")
                    else:
                        self.logger.log("error", f"  - Kaynak: {conflict['source_package']} {conflict['source_version']}, Talep: {conflict['conflicting_dependency']}")
            return {"action": "retry_with_args", "new_args": ["--use-feature=2020-resolver"], "reason": "Tespit edilen bağımlılık çakışması için yeni çözücü deneniyor."}
        ...
        if error_type == "network_error":
            return {"action": "retry_with_delay", "delay": 15, "reason": "Ağ hatası tespit edildi. 15 saniye sonra yeniden denenecek."}

        if error_type == "permission_error":
            return {"action": "retry_with_args", "new_args": ["--user"], "reason": "İzin hatası tespit edildi. --user bayrağı ile yeniden denenecek."}

        if error_type in ["missing_visual_cpp", "cl_exe_failed"]:
            return {"action": "abort", "reason": "Microsoft C++ Build Tools gerekli. https://visualstudio.microsoft.com/visual-cpp-build-tools/ adresinden kurun."}

        raw_error = details.get("raw", "Detay yok")
        return {"action": "abort", "reason": f"Çözümlenemeyen hata: {error_type}. Detay: {raw_error[:200]}..."}
...
class SmartInstallManager:
    ...
    def run_all(self):
        ...
        # Optimizasyon sadece bağımlılıklar biliniyorsa anlamlıdır
        known_modules = [m for m in prioritized_queue if self.installer.dependency_registry.get_package_info(m)]
        optimized_order = self.installer.dependency_optimizer.get_optimized_order(known_modules)
        
        # Optimize edilmemiş modülleri sıranın sonuna ekle
        unknown_modules = [m for m in prioritized_queue if m not in known_modules]
        final_queue = optimized_order + unknown_modules
    ...
    def prioritize_adaptive(self) -> List[str]:
        ...
        def weight(mod: str) -> int:
            w = 10
            if self.installer.heuristic_manager.is_foundational(mod):
                w -= 5
            if self.installer.dependency_registry.get_package_info(mod):
                w -= 2
    ...
    async def schedule_installation(self, initial_queue: List[str]):
        processed_packages = set()
        queue = deque(initial_queue)

        while queue:
            packages_to_process_this_round = list(queue)
            queue.clear()

            tasks = []
            for mod in packages_to_process_this_round:
                if mod in processed_packages:
                    continue
                processed_packages.add(mod)

                # install_packages içindeki kontrolü burada da yap
                pkg_name_only = mod.split("==")[0].split("[")[0]
                if self.installer.mode != 'force' and self.installer.check_package_installed(pkg_name_only):
                    self.logger.log("info", f"Yeni keşfedilen bağımlılık '{pkg_name_only}' zaten kurulu, atlanıyor.")
                    self.installer.summary_generator.add_module_status(pkg_name_only, "Atlandı", 0, reason="Zaten yüklü (yeni keşfedildi)")
                    continue
            ...
            if new_dependencies:
                # Henüz işlenmemiş yeni bağımlılıkları kuyruğa ekle
                unique_new_deps = {dep for dep in new_dependencies if dep not in processed_packages}
                if unique_new_deps:
                    self.logger.log("info", f"Yeni bağımlılıklar bulundu: {list(unique_new_deps)}. Kurulum kuyruğuna ekleniyor...")
                    for dep in unique_new_deps:
                        queue.append(dep)
    ...
    def _install_package_with_retry(self, package: str, max_retries: int = 3) -> List[str]:
        ...
        while retries <= max_retries:
            ...
            if action == "retry_with_args":
                ...
                # Argümanın zaten ekli olup olmadığını kontrol et
                if all(arg not in current_args for arg in new_args):
                    current_args.extend(new_args)
                    self.logger.log("info", f"Yeni deneme için argümanlar güncellendi: {current_args}")
                else:
                    self.logger.log("warning", f"Argümanlar zaten mevcut: {new_args}. Bu hatayı atlıyorum.")
                    action = "abort" # Sonsuz döngüyü engelle

            elif action == "retry_with_delay":
                delay = resolution.get("delay", 10)
                self.logger.log("info", f'{resolution.get("reason")} {delay} saniye bekleniyor...')
                time.sleep(delay)
