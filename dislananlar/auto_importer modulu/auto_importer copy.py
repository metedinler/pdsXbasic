...
import zipfile
import json  # added to support JSON operations
...
CORE_DEPENDENCIES = {
    "base": [
        ...
        "tensorflow",    # added to ensure TensorFlow is installed for core2-6
        "psycopg2-binary",
        "pyyaml",
...
def ensure_venv():
    # rem #13.1. Base ortamdaysa otomatik olarak izole venv'ye geçiş
    # rem #13.2. venv yoksa oluştur, pip'i güncelle, scripti tekrar başlat
...
def install_missing_packages():
    # rem #19.1. pip cache temizliği
    """Gerekli pip paketlerini paralel olarak yükler (sürüm sabitli, çakışmasız)."""
    import importlib
    import glob
    import sys
    import shutil
    ...
    # rem #19.2. Ana ML/bilimsel paketlerin toplu kurulumu
    print("[PDS-X] Ana ML/bilimsel paketler tek seferde kuruluyor (numpy==1.26.4, scipy==1.11.4, scikit-learn==1.3.2 uyumlu)...")
    run_subprocess_logged([sys.executable, "-m", "pip", "install", "--force-reinstall", "--no-cache-dir"] + main_pkgs)

    summary = []
    lock = threading.Lock()

    def install_one(pip_name, import_name):
        # rem #19.3. Diğer paketlerin paralel kurulumu
        # Ana ML kütüphaneleri yukarıda yüklendi, tekrar yükleme
        if any(pip_name.split("==")[0] == p.split("==")[0] for p in main_pkgs):
        ...
        if import_name == "torch_geometric":
            try:
                ...
                return (pip_name, "ATLANDI")
        if import_name in ["markdown_it_py", "torch"]:
            print(f"[PDS-X] {import_name} için tam uninstall + reinstall başlatılıyor...")
            run_subprocess_logged([sys.executable, "-m", "pip", "uninstall", "-y", pip_name.split("==")[0]])
            run_subprocess_logged([sys.executable, "-m", "pip", "install", pip_name, "--no-cache-dir", "--force-reinstall"])
        try:
            ...
            result = run_subprocess_logged([sys.executable, "-m", "pip", "install", pip_name, "--no-deps", "--force-reinstall", "--no-cache-dir"])
            try:
                __import__(import_name)
                return (pip_name, "OK")
            except ImportError as e2:
                if import_name == "textblob" and "nltk" in str(e2):
                    print("[PDS-X] textblob için nltk eksik, otomatik yükleniyor...")
                    run_subprocess_logged([sys.executable, "-m", "pip", "install", "nltk", "--force-reinstall", "--no-cache-dir"])
                    try:
                        __import__(import_name)
                        return (pip_name, "OK")
                    except ImportError as e3:
                        print(f"[PDS-X] textblob import HATASI (nltk sonrası): {e3}")
                        return (pip_name, "HATA")
                print(f"[PDS-X] Paket import edilemiyor, tekrar deneniyor (upgrade): {pip_name}")
                result2 = run_subprocess_logged([sys.executable, "-m", "pip", "install", pip_name, "--upgrade", "--force-reinstall", "--no-cache-dir"])
                try:
                    __import__(import_name)
                    return (pip_name, "OK")
                except ImportError as e4:
                    print(f"[PDS-X] Paket import HATASI: {pip_name}\n{e4}")
                    return (pip_name, "HATA")
    ...
    # rem #19.5. Sonuç özetinin yazdırılması
...
class IsolatedEnvManager:
    ...
    def _cleanup_old_envs(self):
        ...
        old_names = [
            ".pdsx_isolated_env",  # Küçük x olan eski isim
            "pdsx_isolated_env",   # Nokta olmayan eski isim
    ...
    def import_module(self, module_name: str) -> Any:
        ...
        try:
            ...
            if module_name in MODULE_SPECIFIC_DEPS:
                ...
                conflicts = self.env_manager.check_package_conflicts(deps)
                
                if conflicts:
                    print(f"[PDS-X] Paket çakışması tespit edildi: {conflicts}")
                    return None
