...
# Version: 1.7.9.4.001- Terminal Log Analizi, Argparse Replay, Bağımlılık Öğrenme
# Date: June 23, 2025
...
class GracefulShutdownManager:
    ...
    def cleanup(self):
        ...
        - Süreçler: subprocess.Popen ile başlatılan tüm süreçler sonlandırılır (5 sn timeout ile).
        - Asenkron Görevler: asyncio görevleri iptal edilir ve olay döngüsü kapatılır.
        - Temizlik Fonksiyonları: Kayıtlı fonksiyonlar (örneğin, önbellek temizleme) çalıştırılır.
        """
        try:
...
class EnvManager:
    ...
    def update_pip_if_needed(self, force_latest: bool = False, silent: bool = False) -> bool:
        try:
            ...
            process = subprocess.run(install_args, capture_output=True, text=True)
            shutdown_manager.register_process(process)
            ...
            if process.returncode == 0:
                if self.test_pip_compatibility():
                    ...
                    process = subprocess.run([pip_cmd, "install", "pip==21.2.4"], check=True, capture_output=True, text=True)
                    shutdown_manager.register_process(process)
                    return True
            else:
    ...
    def test_pip_compatibility(self):
        try:
            pip_cmd = str(self.venv_dir / ("Scripts" if os.name == "nt" else "bin") / "pip")
            process = subprocess.run([pip_cmd, "check"], capture_output=True, text=True)
            shutdown_manager.register_process(process)
    ...
    def setup_environment(self) -> bool:
        ...
        try:
            ...
            if not self.venv_dir.exists():
                ...
                process = subprocess.run([python_path, "-m", "venv", str(self.venv_dir)], check=True)
                shutdown_manager.register_process(process)
    ...
    def check_multiple_packages(self, packages):
        try:
            ...
            process = subprocess.run([venv_python, "-c", imports], capture_output=True, text=True)
            shutdown_manager.register_process(process)
            ...
            while attempt < max_attempts:
                ...
                if self.check_multiple_packages(REQUIRED_PACKAGES):
                    ...
                    for package_spec, import_name in REQUIRED_PACKAGES:
                        try:
                            venv_python = str(self.venv_dir / ("Scripts" if os.name == "nt" else "bin") / "python")
                            process = subprocess.run([venv_python, "-c", f"import {import_name}"], capture_output=True, text=True)
                            shutdown_manager.register_process(process)
                            if process.returncode == 0:
...
# auto_importer.py - PDS-X Akıllı Modül Yükleyici
# Version: 1.7.9.4 - Terminal Log Analizi, Argparse Replay, Bağımlılık Öğrenme
# Date: June 23, 2025
# Author: xAI (Grok 3 ile oluşturuldu, fikir Mete Dinler, düzenleyen GitHub Copilot)
...
class ConflictManager:
    ...
    def detect_conflicts(self, module_name: str, deps: List[str]) -> Dict:
        try:
            ...
            conflicts = {}
            for dep in deps:
                process = subprocess.run([sys.executable, "-m", "pip", "check"], capture_output=True, text=True)
                shutdown_manager.register_process(process)
                if "no conflicts" not in process.stdout.lower():
    ...
    def quantum_analysis(self, module_deps: Dict) -> Dict:
        ...
        NumPy ile bağımlılık matrisi oluşturur ve istatistiksel analiz yapar.
        """
        try:
            np = get_numpy()
            if np is None:
...
class AutoImporter:
    ...
    def __init__(self):
        if not hasattr(self, 'initialized'):
            try:
                ...
                self.metadata = {"auto_importer": {"version": "1.7.9.4", "dependencies": []}}
    ...
    def cleanup_on_shutdown(self):
        """
        Shutdown sırasında temizlik yapar: Önbelleği ve kayıtları kaydeder, aktif süreçleri sonlandırır.
        """
        try:
    ...
    def check_stop_combination(self, event):
        """
        Ctrl+Shift+Q kombinasyonu ile programı güvenli şekilde durdurur.
        """
        try:
            keyboard = get_keyboard()
    ...
    def save_last_args(self, args: List[str]):
        ...
        try:
            with open(LAST_ARGS_FILE, "w", encoding="utf-8") as f:
                json.dump({"command": args, "timestamp": datetime.now().isoformat()}, f, indent=4)
            self.logger.log("info", f"Komut argümanları kaydedildi: {args}")
        except Exception as e:
            self.logger.log("error", f"Komut argümanları kaydetme hatası: {e}")

    def load_last_args(self) -> Optional[List[str]]:
        """
        .pdsx_last_args.json’dan kaydedilmiş argümanları yükler.
        """
        try:
    ...
    def enable_offline_mode(self):
        """
        Çevrimdışı modu etkinleştirir ve requirements.txt’yi önbelleğe indirir.
        """
        try:
    ...
    def install_package(self, package: str, silent=False):
        """
        Paketi kurar, önbellekten yükler, çakışmaları kontrol eder ve özet üretir.
        """
        try:
            log_level = "debug" if silent else "info"
            ...
            if self.dependency_registry.check_package(package):
                ...
                self.summary_generator.add_module_status(package, "Atlandı", 0.0)
                return
            attempts = 0
            max_attempts = 3
            while attempts < max_attempts and self.running:
                try:
                    ...
                    if process.returncode == 0:
                        ...
                        self.dependency_registry.register_package(package, package.split("==")[1] if "==" in package else "latest", "Başarılı")
                        break
                    if not self.pip_analyzer.analyze_and_fix(process.stderr, package):
                        raise Exception(f"{package} kurulum hatası: {process.stderr}")
                except Exception as e:
                    attempts += 1
                    if not silent:
                    ...
                    if attempts == max_attempts:
                        ...
                        self.summary_generator.add_module_status(package, f"Hata: {str(e)}", time.time() - start_time)
                        return
                    time.sleep(2 ** attempts)
            conflicts = self.conflict_manager.detect_conflicts(package, [package])
            if conflicts:
                resolutions = self.conflict_manager.resolve_conflicts(package, conflicts, self.dependency_registry)
    ...
    async def async_install_package(self, package: str):
        """
        Paketi asenkron olarak kurar, önbellekten yükler, çakışmaları kontrol eder.
        """
        try:
            ...
            if self.cache_manager.install_from_cache(package):
                self.record_installation_attempt(package, True)
                self.dependency_registry.register_package(package, package.split("==")[1] if "==" in package else "latest", "Başarılı")
                self.summary_generator.add_module_status(package, "Başarılı", 0.0)
                return
            ...
            if process.returncode == 0:
                ...
                self.record_installation_attempt(package, False)
                self.dependency_registry.register_package(package, package.split("==")[1] if "==" in package else "latest", "Başarısız")
                self.summary_generator.add_module_status(package, f"Hata: {stderr.decode()}", 0.0)
            conflicts = self.conflict_manager.detect_conflicts(package, [package])
            if conflicts:
                resolutions = self.conflict_manager.resolve_conflicts(package, conflicts, self.dependency_registry)
                self.dependency_registry.register_resolution(package, resolutions)
            ...
            self.summary_generator.add_module_status(package, f"Hata: {str(e)}", 0.0)

    def load_module(self, file_path: str, alias: Optional[str] = None) -> Any:
        """
        Python modülünü yükler, bağımlılıklarını kurar ve özet üretir.
        """
        with self.lock:
            try:
    ...
    def run(self, packages: List[str]):
        """
        Verilen paketleri kurar, çakışmaları kontrol eder ve özet üretir.
        """
        try:
            self.logger.log("info", "Kütüphane yükleme başlatıldı.")
            conflicts = self.conflict_manager.detect_conflicts("pdsX", packages)
            if conflicts:
                resolutions = self.conflict_manager.resolve_conflicts("pdsX", conflicts, self.dependency_registry)
    ...
    def is_recently_installed(self, module_name: str, timeout_minutes: int = 30) -> bool:
        """
        Modülün yakın zamanda başarıyla kurulup kurulmadığını kontrol eder.
        """
        try:
    ...
    def should_retry_install(self, module_name: str, max_retries: int = 3) -> bool:
        """
        Modülün yeniden denenip denenmeyeceğini kontrol eder.
        """
        try:
    ...
    def record_installation_attempt(self, module_name: str, success: bool):
        """
        Kurulum denemesini kaydeder ve yeniden deneme sayısını günceller.
        """
        try:
            if success:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PDS-X AutoImporter")
    parser.add_argument("--interactive", action="store_true", help="REPL modunda çalıştır")
    parser.add_argument("script", nargs="?", help="Çalıştırılacak .basX dosyası")
    args = parser.parse_args()
    
    importer = AutoImporter()
    importer.save_last_args(sys.argv)
    
    if importer.env_manager.setup_environment():
        last_args = importer.load_last_args()
        if last_args:
            importer.logger.log("info", f"Son komut tekrar oynatılıyor: {last_args}")
            importer.env_manager.restart_in_venv(last_args)
        else:
            importer.logger.log("info", "Komut bulunamadı, varsayılan modda devam ediliyor.")
            if args.interactive:
                importer.load_module("pdsXuv14.py")
            elif args.script:
                importer.load_module(args.script)
    else:
        importer.logger.log("error", "Ortam hazırlama başarısız, çıkış yapılıyor.")
        sys.exit(1)