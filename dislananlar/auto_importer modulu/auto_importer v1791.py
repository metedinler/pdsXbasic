...
class EnvManager:
    ...
    def setup_environment(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run([pip_cmd, "install", "-U", "pip"], capture_output=True, text=True)
                # Graceful shutdown kontrolü
                if shutdown_manager.shutdown_requested:
                    self.logger.log("info", "Shutdown sinyali alındı, pip işlemi durduruluyor...")
                    return False
                    
                if result.returncode != 0:
                    self.logger.log("warning", f"Pip güncellemesi başarısız: {result.stderr}")
            except Exception as e:
                self.logger.log("warning", f"Pip güncellemesi başarısız: {e}")
            ...
            self.logger.log("info", "Diğer paketler yükleniyor...")
            essential_packages = [
                "scipy==1.11.4", "pandas==2.1.4", "scikit-learn==1.3.2", 
                "matplotlib==3.8.4", "psutil"
            ]
            for package in essential_packages:
                self.logger.log("info", f"Yükleniyor: {package}")
                try:
                    subprocess.run([pip_cmd, "install", package], check=True, capture_output=True)
                except subprocess.CalledProcessError as e:
                    self.logger.log("warning", f"{package} yüklenemedi: {e}")
                    continue
    ...
    def is_running_in_venv(self) -> bool:
        ...
        try:
            ...
            # Yol karşılaştırması
            return os.path.normpath(current_python) == os.path.normpath(venv_python)
        except Exception as e:
