...
Yazar: xAI (Gemini ile oluşturuldu, fikir Mete Dinler, düzenleyen GitHub Copilot-Geminni)
"""

import sys
import io

# Windows'ta konsol karakter kodlaması sorunlarını çözmek için stdout ve stderr'i yeniden yapılandır.
if sys.platform == "win32":
    try:
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
        sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8', errors='replace')
        # Bu satır, print() fonksiyonunun doğrudan UTF-8 kullanmasını sağlar.
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')
    except (TypeError, ValueError) as e:
        # Bu hata, betik bir IDE içinde veya zaten uyumlu bir terminalde çalıştırıldığında oluşabilir.
        # Bu durumda, genellikle ek bir yapılandırmaya gerek yoktur.
        print(f"[PDS-X UYARI] Standart akışlar yeniden yapılandırılamadı: {e}", file=sys.__stderr__)
...
class DependencyRegistry:
    ...
    def _normalize_name(self, package_name: str) -> str:
        """Paket adını PEP 503 normallerine göre standartlaştırır (küçük harf, _ -> -)."""
        if not package_name:
            return ""
    ...
    def add_package(self, package_name: str, version: str, dependencies: List[str]):
        """Kayıtlara yeni bir paket ekler veya mevcut olanı günceller."""
    ...
    def get_package_info(self, package_name: str) -> Optional[Dict[str, Any]]:
        """Normalleştirilmiş bir paket adı için bilgi döndürür."""
        normalized_name = self._normalize_name(package_name)
...
class ModuleSummaryGenerator:
    ...
    def print_and_log_summary(self):
        """Tamamlanan tüm kurulumlar için son bir özet oluşturur ve yazdırır."""
        if self.mode_manager.is_silent:
            return

        total_duration = time.time() - self.start_time
        successful_installs = [r for r in self.results if r["status"] in ["Başarılı", "Atlandı"]]
        failed_installs = [r for r in self.results if r["status"] == "Başarısız"]

        # Renkleri al (colorama yüklü değilse boş string döndür)
        ...
        summary = [
            ...
            f"Toplam {len(self.results)} paket işlendi. Toplam Süre: {total_duration:.2f} saniye.",
            f"{Fore.GREEN}Başarılı Kurulumlar ({len(successful_installs)}):{Style.RESET_ALL}",
            "  " + (', '.join([r['package'] for r in successful_installs]) or "Yok"),
            f"{Fore.RED}Başarısız Kurulumlar ({len(failed_installs)}):{Style.RESET_ALL}",
        ]
        ...
        final_summary_text = "\n".join(summary)
        # Bu loglama hem dosyaya (JSON olmayan formatta) hem de konsola yazdırır.
    ...
    def shutdown(self):
        ...
        for task in reversed(self.tasks):
            try:
                ...
                # Hata nesnesini string'e çevirerek JSON serileştirme sorununu çöz
...
class SmartInstallManager:
    """
    Chaos analizi, adaptif önceliklendirme, asenkron kurulum, bağımlılık optimizasyonu,
    blockchain tabanlı doğrulama ve çakışma geri alma adımlarını içerir.
    """
    ...
    def analyze_chaos(self, duration: int = 10):
        ...
        status = (
            "Düşük Yük (Kararlı)" if chaos_index < 3 else
            "Dalgalı Yük (Orta)" if chaos_index < 7 else
            "Kaotik Yük (Yüksek)"
        )
        try:
    ...
    # Adım 2: Adaptive Önceliklendirme
    def prioritize_adaptive(self):
        level = self.chaos_report.get("status", "Düşük Yük (Kararlı)")
        def weight(mod: str) -> int:
            ...
            if level == "Kaotik Yük (Yüksek)":
                w += 2
            elif level == "Dalgalı Yük (Orta)":
                w += 1
            return w
        self.priority_queue = sorted(self.modules, key=weight)
        self.installer.logger.log("info", f"📦 Adaptive Sıralama: {self.priority_queue}")

    # Adım 3: Dependenciy Optimizasyonu
    def optimize_dependencies(self):
        graph = nx.DiGraph()
        # Basit örnek bağımlılık haritası
        dep_map = {
        ...
        try:
            ...
            self.installer.logger.log("warning", "⚠️ Döngüsel bağımlılık tespit edildi.")
            self.dependency_order = self.priority_queue
        self.installer.logger.log("info", f"📐 Dependency Sırası: {self.dependency_order}")

    # Adım 4: Asenkron Kurulum
    ...
    async def install_with_delay(self, mod: str, delay: float):
        await asyncio.sleep(delay)
        try:
            self.installer.install_package(mod)
        except Exception as e:
            self.installer.logger.log("error", f"Kurulum hatası ({mod}): {e}")
    ...
    # Adım 5: Çakışma Tespiti ve Geri Dönüş
    def detect_conflicts_and_rollback(self):
        """pip check ile bağımlılıkları kontrol eder ve çakışma varsa geri almaya çalışır."""
        if not self.installer.env_manager.pip_path:
            self.installer.logger.log("error", "Pip yolu bulunamadığı için çakışma kontrolü yapılamıyor.")
            return
            
        cmd = [str(self.installer.env_manager.pip_path), "check"]
        self.installer.logger.log("info", f"Bağımlılık kontrolü çalıştırılıyor: {' '.join(cmd)}")
        
        result = subprocess.run(cmd, capture_output=True, text=True, encoding='utf-8', errors='ignore')
        
        if result.returncode != 0:
            self.installer.logger.log("warning", f"pip check komutu potansiyel çakışmalar buldu:\n{result.stdout}")
            for line in result.stdout.splitlines():
                if not line.strip():
                    continue
                self.installer.logger.log("warning", f"🚨 Çakışma: {line}")
                # pip check çıktısı 'package-name version has requirement other-package ...' şeklinde olabilir.
                pkg = line.split()[0]
                prev = self.get_previous_version(pkg)
                if prev:
                    self.installer.logger.log("info", f"↩️ Geri alınıyor: {pkg}=={prev} kuruluyor...")
                    try:
                        ...
                        self.installer.logger.log("info", f"Geri alma başarılı ({pkg}=={prev}): {rollback_result.stdout}")
                    except subprocess.CalledProcessError as e:
                        self.installer.logger.log("error", f"Geri alma başarısız ({pkg}=={prev}): {e.stderr}")
        else:
            self.installer.logger.log("info", "✅ pip check: Bağımlılık çakışması bulunamadı.")


    def get_previous_version(self, pkg: str) -> Optional[str]:
        """Bir paketin kayıtlı bir önceki versiyonunu döndürür."""
        try:
            package_info = self.installer.dependency_registry.get_package_info(pkg)
            if not package_info:
                self.installer.logger.log("warning", f"'{pkg}' için kayıt bulunamadı, önceki versiyon alınamıyor.")
                return None
            ...
            if len(versions) > 1:
                ...
                self.installer.logger.log("info", f"'{pkg}' için sadece bir versiyon kaydı var, geri alınacak versiyon yok.")
                return None
        except (AttributeError, IndexError, TypeError) as e:
            self.installer.logger.log("error", f"'{pkg}' için önceki versiyon alınırken kritik hata: {e}")
            return None
...
class AutoImporter:
    ...
    def _install_package_with_retry(self, package: str, max_retries: int = 3) -> List[str]:
        ...
        while retries < max_retries:
            ...
            action_plan = self.pip_output_analyzer.analyze(output)
    ...
    def import_and_install(self, module_name: str, package_name: str = None, version: str = None):
        """
        Bir modülü içe aktarmayı dener, başarısız olursa otomatik olarak kurar.
        Bu, PDS-X ana döngüsünden çağrılacak ana fonksiyondur.
        """
        if not self.auto_install_enabled:
        ...
        try:
            ...
            self.logger.log("warning", f"Modül '{module_name}' bulunamadı. SmartInstallManager ile kurulacak.")
            pkg = self._resolve_package_name(module_name, package_name)
            if version:
                pkg += f"=={version}"
            
            # SmartInstallManager ile asenkron ve adaptif kurulum
            manager = SmartInstallManager([pkg], self)
            manager.run_all()
    ...
    def install_from_list(self, modules: List[str]):
        """Verilen bir modüller listesini kurar."""
        self.logger.log("info", f"{len(modules)} modülden oluşan liste için SmartInstallManager ile toplu kurulum başlatılıyor.")
        manager = SmartInstallManager(modules, self)
        manager.run_all()
        self.logger.log("info", "SmartInstallManager ile toplu kurulum tamamlandı.")
    ...
    def trigger_task(self, task: Dict[str, Any]):
        ...
        self.logger.log("info", f"Harici görev alındı: {task_name}")

        if task_name == "install_list":
            if isinstance(payload, list):
                self.install_from_list(payload)
            else:
                self.logger.log("error", f"install_list görevi için payload liste olmalı, alındı: {type(payload)}")
        elif task_name == "install_file":
            if isinstance(payload, str):
                self.install_from_file(payload)
            else:
                self.logger.log("error", f"install_file görevi için payload dosya yolu (str) olmalı, alındı: {type(payload)}")
        else:
            self.logger.log("warning", f"Bilinmeyen görev alındı: {task_name}")
        ...
        try:
            ...
            while not self.installation_queue.empty():
                ...
                try:
                    ...
                    # Hatalı paketi tekrar denemek için işaretle
...
def main():
    ...
    if TERMINAL_LOG_FILE.exists():
        try:
            TERMINAL_LOG_FILE.write_text('')  # Log dosyasını temizle
        except Exception as e:
    ...
    try:
        ...
        # Kurulumlar tamamlanana kadar bekle ve ardından kapat
