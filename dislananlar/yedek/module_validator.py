...
def validate_exports(module_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not hasattr(module, "__pdsX_exports__"):
            raise PdsXException(f"{module_name} modülünde __pdsX_exports__ tanımı yok")
        ...
        for key in required_keys:
            if key not in exports:
                raise PdsXException(f"{module_name} modülünün __pdsX_exports__ yapısında {key} anahtarı eksik")
        
        # Sınıfları kontrol et
        for class_name, class_obj in exports["classes"].items():
            if not isinstance(class_obj, type):
                raise PdsXException(f"{module_name} modülünde {class_name} geçerli bir sınıf değil")
        
        # Fonksiyonları kontrol et
        for func_name, func_obj in exports["functions"].items():
            if not callable(func_obj):
                raise PdsXException(f"{module_name} modülünde {func_name} geçerli bir fonksiyon değil")
