"""
###########################################
# PDS-X BASIC v14u Exception Handler
###########################################
#
# Bu modül, PDS-X framework'ünün hata yönetim sistemini implemente eder.
# Temel görevleri:
# 1. Özelleştirilmiş hata tipleri
# 2. Hata yakalama ve işleme
# 3. Hata loglama ve raporlama
# 4. Hata geri yükleme (recovery)
# 5. Debug bilgisi toplama
#
...
class ExceptionManager:
    ...
    def unregister_handler(self, error_code: str, handler: str) -> None:
        ...
        with self.lock:
            try:
                if (error_code.upper() in self.error_handlers and 
                    handler in self.error_handlers[error_code.upper()]):
    ...
    def get_error_log(self, max_entries: Optional[int] = None) -> List[Dict]:
        ...
        with self.lock:
            try:
                return ([error for _, error in self.error_log[-max_entries:]] 
                        if max_entries else [error for _, error in self.error_log])
            except Exception as e:
