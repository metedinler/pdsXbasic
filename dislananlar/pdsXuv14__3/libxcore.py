...
class LibXCore:
    ...
    def merge(self, lst1: List, lst2: List, key: Optional[str] = None) -> List:
        """İki listeyi birleştirir."""
        if key is None:
            return lst1 + lst2
        else:
            return [*{item[key]: item for item in lst1 + lst2}.values()]
            
    def sort(self, collection: List, key: Optional[str] = None, reverse: bool = False) -> List:
        """Koleksiyonu sıralar."""
        if key is None:
