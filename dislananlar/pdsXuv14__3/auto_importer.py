...
def install_missing_packages():
    """Gerekli pip paketlerini yükler (sürüm sabitli, çakışmasız)."""
    import importlib
    import glob
    import sys
    import shutil
    ...
    # 1. pip cache temizle (eski wheel çakışmalarını önle)
    print("[PDS-X] pip cache temizleniyor...")
    subprocess.run([sys.executable, "-m", "pip", "cache", "purge"], check=False)

    # 2. Ana ML/bilimsel paketler (numpy==1.26.4 sabit, diğerleriyle birlikte tek seferde kur)
    main_pkgs = [
    ...
    print("[PDS-X] Ana ML/bilimsel paketler tek seferde kuruluyor (numpy==1.26.4 sabit)...")
    subprocess.run([sys.executable, "-m", "pip", "install"] + main_pkgs, check=False)

    # 3. Diğer küçük paketleri --no-deps ile yükle
    summary = []
