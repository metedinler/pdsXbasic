...
# Version: 1.7.9.3 - Düzeltmeler ve İyileştirmeler
# Date: June 23, 2025
        ...
        try:
            ...
            if Elasticsearch:
                try:
                    ...
                    self.logger.log("warning", f"Elasticsearch sunucusu erişilemez, yerel loglamaya geçiliyor: {e}")
...
class EnvManager:
        ...
        try:
            ...
            if process.returncode == 0:
                if self.test_pip_compatibility():
                    ...
                    subprocess.run([pip_cmd, "install", "pip==21.2.4"], check=True, capture_output=True, text=True)
                    return True
            else:
    ...
    def test_pip_compatibility(self):
        try:
            pip_cmd = str(self.venv_dir / ("Scripts" if os.name == "nt" else "bin") / "pip")
            result = subprocess.run([pip_cmd, "check"], capture_output=True, text=True)
    ...
    def setup_environment(self) -> bool:
        try:
            ...
            if self.check_multiple_packages(REQUIRED_PACKAGES):
                ...
                success_count = len(REQUIRED_PACKAGES)
            else:
                for package_spec, import_name in REQUIRED_PACKAGES:
                    ...
                    try:
                        ...
                        continue
            self.logger.log("info", f"Paket kurulum tamamlandı: {success_count}/{len(REQUIRED_PACKAGES)} başarılı")
    ...
    def check_multiple_packages(self, packages):
        try:
            ...
            test_result = subprocess.run([venv_python, "-c", imports], capture_output=True, text=True)
    ...
    def __init__(self):
        if not hasattr(self, 'initialized'):
            try:
                ...
                self.metadata = {"auto_importer": {"version": "1.7.9.3", "dependencies": []}}
