...
class AdvancedLogger:
    ...
    def _setup_logging(self):
        ...
        # Formatter'lar - Terminal için özel format
        file_formatter = logging.Formatter(
            '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
        )
        
        # Terminal için sadece [PDS-X] formatı
        console_formatter = logging.Formatter(
            '[PDS-X] AutoImporter -%(levelname)s- %(message)s'
        )
        ...
        # File handler - Timestamp'li format
        file_handler = logging.FileHandler(log_file, encoding='utf-8')
        file_handler.setFormatter(file_formatter)
        self.logger.addHandler(file_handler)
        
        # Console handler - Temiz format (silent mode kontrolü)
        if not self.silent:
    ...
    def log(self, level, message, extra_data=None):
        ...
        # [PDS-X] prefix'i ekle (sadece terminal için)
        terminal_message = f"[PDS-X] AutoImporter -{level}- {message}"
        
        # Terminal'e doğrudan print et
        print(terminal_message)
        
        # JSONL logging (dosyaya)
    ...
    def restart_in_venv(self) -> bool:
        ...
        try:
            if not self.venv_dir.exists():
                print("[PDS-X] Sanal ortam bulunamadı!")
                return False
            
            venv_python = str(self.venv_dir / "Scripts" / "python.exe") if os.name == "nt" else str(self.venv_dir / "bin" / "python")
            
            if not os.path.exists(venv_python):
                print(f"[PDS-X] Sanal ortam Python'u bulunamadı: {venv_python}")
                return False
            
            print("[PDS-X] Program sanal ortamda yeniden başlatılıyor...")
            
            # Kaydedilmiş argümanları kullan, yoksa mevcut argümanları kullan
            try:
                from pathlib import Path
                import json
                
                args_file = Path(".pdsx_last_args.json")
                if args_file.exists():
                    with open(args_file, "r", encoding="utf-8") as f:
                        saved_data = json.load(f)
                    saved_args = saved_data.get("args", [])
                    if saved_args:
                        print(f"[PDS-X] Kaydedilmiş argümanlar kullanılıyor: {saved_args}")
                        new_cmd = [venv_python, sys.argv[0]] + saved_args
                    else:
                        new_cmd = [venv_python] + sys.argv
                else:
                    new_cmd = [venv_python] + sys.argv
            except Exception as e:
                print(f"[PDS-X] Argüman okuma hatası, mevcut argümanlar kullanılıyor: {e}")
                new_cmd = [venv_python] + sys.argv
            
            print(f"[PDS-X] Yeni komut: {' '.join(new_cmd)}")
            
            # Mevcut process'i sonlandır ve yeni process'i başlat
            os.execv(venv_python, new_cmd)
            
        except Exception as e:
            print(f"[PDS-X] Sanal ortamda yeniden başlatma hatası: {e}")
            return False
...
class RealTimeLogMonitor:
    ...
    def __init__(self, logger, jsonl_file=None):
        ...
        self.repl_mode_detected = False  # REPL modu tespit bayrağı
    ...
    def detect_repl_mode(self):
        """REPL modunu tespit et"""
        try:
            # Komut satırı argümanlarını kontrol et
            if '--interactive' in sys.argv or '--repl' in sys.argv:
                return True
            
            # PDS-X REPL prompt'u kontrol et
            if hasattr(sys, 'ps1') or 'PDS-X>' in str(sys.argv):
                return True
                
            # Etkileşimli ortam kontrolü
            if hasattr(sys, 'ps1') or hasattr(sys.stdin, 'isatty') and sys.stdin.isatty():
                return True
    ...
    def start_monitoring(self):
        """Log izlemeyi başlat - REPL modunda devre dışı"""
        # REPL modu kontrolü
        if self.detect_repl_mode():
            self.repl_mode_detected = True
            self.logger.info("REPL modu tespit edildi - AutoImporter monitoring devre dışı")
            return
    ...
    def _check_system_performance(self):
        """Sistem performansını kontrol et - REPL modunda devre dışı"""
        # REPL modu kontrolü
        if self.repl_mode_detected or self.detect_repl_mode():
            return  # REPL modunda sistem performans kontrolü yapma
...
class AutoImporter:
    ...
    def __new__(cls, entry_point_script=None, mode="HEAVY", **kwargs):
        # Her seferinde yeni instance döndür
        return super(AutoImporter, cls).__new__(cls)
    
    def __init__(self, entry_point_script=None, mode="HEAVY", **kwargs):
        ...
        # EnvManager'a logger veriyor, bu sefer kendi AdvancedLogger oluşturmasın
...
"""
    ...
    def cleanup_on_shutdown(self):
        """Graceful shutdown cleanup - Tüm kaynakları temizle"""
        try:
            self.logger.info("🛑 AutoImporter cleanup başlatıldı...")
            
            # 1. Monitoring sistemi durdur
            if hasattr(self, 'realtime_monitor'):
                try:
                    self.realtime_monitor.stop_monitoring()
                    self.logger.info("✅ Real-time monitoring durduruldu")
                except Exception as e:
                    self.logger.warning(f"⚠️ Monitoring durdurma hatası: {e}")
            
            # 2. Thread pool'u kapat
            if hasattr(self, 'thread_pool'):
                try:
                    self.thread_pool.shutdown(wait=True)
                    self.logger.info("✅ Thread pool kapatıldı")
                except Exception as e:
                    self.logger.warning(f"⚠️ Thread pool kapatma hatası: {e}")
            
            # 3. Dosya handle'larını kapat
            ...
            if hasattr(self, 'cache_manager'):
                try:
                    ...
                    self.logger.warning(f"⚠️ Cache temizleme hatası: {e}")
            ...
            if hasattr(self, 'summary_generator'):
                try:
                    ...
                    self.logger.warning(f"⚠️ Final rapor hatası: {e}")
            
            # 6. Tüm background işlemlerini durdur
            self.running = False
            ...
            # En azından running'i false yapalım
            self.running = False
    
    def shutdown(self):
        """AutoImporter'ı güvenli şekilde kapat - Tüm task'ları ve thread'leri durdur"""
        try:
            self.logger.info("[PDS-X] AutoImporter görevini tamamladı, graceful shutdown başlatılıyor...")
            
            # 1. Running durumunu false yap (ilk adım)
            self.running = False
            
            # 2. Monitoring'i durdur
            if hasattr(self, 'realtime_monitor'):
                self.realtime_monitor.stop_monitoring()
                self.logger.info("[PDS-X] ✅ Real-time monitoring durduruldu")
            
            # 3. Tüm aktif thread'leri durdur
            if hasattr(self, 'thread_pool'):
                self.thread_pool.shutdown(wait=True)
                self.logger.info("[PDS-X] ✅ Thread pool kapatıldı")
            
            # 4. Global shutdown manager'ı kullan
            global shutdown_manager
            if shutdown_manager:
                shutdown_manager.request_shutdown()
                shutdown_manager.cleanup()
                self.logger.info("[PDS-X] ✅ Graceful shutdown manager çalıştırıldı")
            ...
            # 7. PDS-X'e devretme mesajı
            self.logger.info("[PDS-X] 🎯 AutoImporter görevi tamamlandı")
            self.logger.info("[PDS-X] 🚀 Sistem kontrolü PDS-X REPL'e devrediliyor...")
